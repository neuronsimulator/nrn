...
export PATH=/opt/homebrew/opt/bison/bin:/opt/homebrew/opt/flex/bin:/opt/homebrew/opt/m4/bin:$PATH
...
packaging/python/build_wheels.bash osx 3.12 coreneuron &>3.12-output

if [ -n $INTERACTIVE_OK ]
then
    for py in 9 10 11 12
    do
...
bash packaging/python/test_wheels.sh python3.12 wheelhouse/NEURON-*-cp312*.whl true &>3.12-test-output

...
**Tree:** `~/neuron/nrngpu`  
**Living tip (2026-09-10):** `local/gpu-native` (master merge `76ba78245` + host-NR SoA slim). Campaign `2026-09-10-tip-psolve-matrix`: 8×4 psolve, identity vs CPU. Dentate nt1 1-rank: CN GPU warm ~**0.53 s** vs native ~**1.58 s** (≈**3.0×**, was 1.80 s / 3.4×; ✅ 400). **`H-dentate-nt1-host-nr-soa` closed:** Gfluct3 `g_e1`/`g_i1` only + deliver-wave coalesce (292 620 → 234 H→D). Residual is t=1 host TQ (`H-dentate-nt1-deliver`). Traub no-gap native ~**1.21×** CN GPU; Traub gap native ✅ 7873 / CN 7867. Ring160 nt1 native **faster** than CN GPU on T1000.  
...
**Next:** Dentate nt1 t=1 host TQ/NetCon (`H-dentate-nt1-deliver`). Warm native ~**1.58 s** vs CN GPU ~**0.53 s** (≈**3.0×**); t=1 still ~0.81 s after host-NR SoA slim. Not density. Not 4-rank MPS. Not ion SoA / net_buf / NSB.

### Starting prompt — Dentate nt1 t=1 host TQ (next)
...
Hypothesis: H-dentate-nt1-deliver — t=1 host TQ/NetCon is ~0.81 s of the
  ~1.58 s warm psolve (CN GPU ~0.53 s). Host-NR SoA H→D closed (292k→234).
  NRB 0.009; STATE/CURRENT ≈ CN. Identity ✅ 400.

This session: attribute and, if ≥ ~0.2 s with a wall hypothesis, recode the
  t=1 deliver-tq path (not NRB, not density). Re-measure 1-rank Dentate nt1
...
Warm 1.80→1.58 s; ACC_TIME 292620→234. ✅ 400. Residual is t=1 host TQ.

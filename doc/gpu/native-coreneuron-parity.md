...
**Tree:** `~/neuron/nrngpu`  
**Living tip (2026-09-11):** `local/gpu-native` (master merge `76ba78245` + host-NR SoA + NRB-fast + device-ensure persist + Traub-gap finitialize gather + nonvint sub-buckets). Campaign `2026-09-11-psolve-setup-warm`: Dentate nt1 1-rank CN GPU Solver ~**0.53 s** vs native warm ~**0.66–0.70 s** (≈**1.24×**; ✅ 400). **`H-dentate-nt1-nonvint` closed:** lastpart-nonvint 0.358 = state 0.355 + prepare 0.002 + finalize 0.0003 — not a host-wait slice of the ~0.13 s. Product warm **0.674–0.675**. Traub gap native ✅ **7873**.  
...
| 2026-09-10 | GPU-P4-dentate-nt1 | 01a08df7 | deliver-tq 0.016 s (not 0.81 s); NRB-fast 3163; first-dt ensure 0.68 s. Next: device-ensure |
| 2026-09-10 | GPU-P4-dentate-nt1 | 01a08ddb | Host-NR SoA slim: live RANGE + coalesce; warm 1.80→1.58 s; 292k→234 H→D; ✅ 400. Next: t=1 TQ |
...
| 2026-09-10 | GPU-P4-dentate-nt1 | 01a08ddb | Host-NR SoA slim on tip: live RANGE (`g_e1`/`g_i1`) + deliver-wave coalesce. Warm **1.58 s** (was 1.80); ACC_TIME 292 620 → **234** updates. ✅ 400; gf_native + watchrange_native green. Next: t=1 host TQ. |
...
| reduced_dentate max_cells=100 tstop=10 **4-rank** | **no MPS** psolve ~**37–38 s**; **with MPS (post H4+E tip)** psolve ~**1.5–1.6 s**, wall ~4.5–5 s | wall ~6.3 s; Solver ~**3.24 s** | no-MPS ~11×; **MPS ≲ CN** | Root cause: multi-process thrash without MPS. Product harness auto-starts MPS. **Spike multiset 400 green** (Eigen `_eigen_global` deviceptr; was 390 vs 400). |
...
7. **Traub product mix (closed 2026-08-04):** identity green. See session ledger GPU-P4-traub.  
   - **no-gap:** QUALIFIED A–F; **4474 exact** native↔CPU↔CN-GPU.  
...
19. **Dentate nt1 exclusive 3.4× (closed 2026-09-11):** 1-rank ×3 psolve, ✅ 400. L1+L2: STATE/CURRENT kernels **≈ CN**. Host-NR SoA slim 1.80→1.58 s. deliver-tq **0.016 s** (not the 0.81 s). **`H-dentate-nt1-device-ensure` closed:** persist GPU mirrors across psolve-end token death; warm **1.56 → 0.68 s** (≈**1.28×** CN 0.53 s); device-ensure 0.68→**0**. **`H-dentate-nt1-nonvint` closed (2026-09-11 re-time):** lastpart-nonvint 0.358 = nonvint-state **0.355** + prepare **0.002** + finalize **0.0003**. Product warm **0.674–0.675**. Remaining named child: setup-rhs ~0.17. Do not reopen density / ion SoA / net_buf / NSB / deliver-tq / device-ensure / nonvint without a new wall hypothesis.
...
**Next:** Dentate nt1 exclusive remaining ~**1.24×** CN GPU (product warm **0.674–0.675** vs Solver 0.530–0.630). **`H-dentate-nt1-nonvint` closed** (state 0.355 / prepare 0.002 / finalize 0.0003). Remaining named child: **`H-dentate-nt1-setup`** (setup-rhs ~0.17). Traub gap persist **closed** (7873). Not 4-rank MPS.

### Starting prompt — Dentate nt1 setup-rhs (next)
...
Kind: feature. Portfolio: GPU-native. Phase: P4 Dentate nt1 exclusive residual.
Tree: ~/neuron/nrngpu. Branch: living tip local/gpu-native.

Context: persist GPU mirrors and Traub gap (native 7873 + CN integer-n 7873)
are closed. H-dentate-nt1-nonvint closed: lastpart-nonvint 0.358 = STATE loop
0.355 + prepare 0.002 + finalize 0.0003 — not a host-wait slice of the ~0.13 s
vs CN. Product warm 0.674–0.675, ✅ 400. Remaining L1: setup-rhs ~0.17;
CURRENT kernels ≈ CN (L2). Do not reopen nonvint / device-ensure / deliver-tq /
host-NR SoA / ion SoA / net_buf / NSB / density without a new wall hypothesis.
Not 4-rank MPS.

This session: H-dentate-nt1-setup only. Recode only if that child is a real
slice of the ~0.13 s vs CN. High performance sacred; CoreNEURON is a guide;
heap-free weight_index.

Commit locally without push. Update Status/Next before exit.
```

### Starting prompt — Dentate nt1 lastpart-nonvint (closed 2026-09-11; archive)

Session closed 2026-09-11: nested prepare/state/finalize. STATE loop 0.355 s;
prepare 0.002; finalize 0.0003. No recode. Product 0.674–0.675 ✅ 400.

### Starting prompt — Traub gap persist (closed 2026-09-11; archive)

Session closed 2026-09-11: finitialize gap gather skipped while persist
mirrors hold previous-psolve V. Traub gap **7873** vs CPU; Dentate persist kept.

### Starting prompt — Dentate nt1 first-step device-ensure (closed 2026-09-11; archive)

Session closed 2026-09-11: persist GPU mirrors across psolve-end token death.
Warm 1.56→0.68 s; device-ensure 0.68→0. ✅ 400. Remaining ~1.28× CN.

### Starting prompt — Dentate nt1 t=1 host TQ (closed 2026-09-10; archive)

Session closed 2026-09-10: deliver-tq 0.016 s after SoA slim (not the 0.81 s).
NRB-fast 3163/3163 NetCon. Residual is first-dt ensure_on_device 0.68 s.

### Starting prompt — Dentate nt1 host-NR SoA H→D (closed 2026-09-10; archive)

Session closed 2026-09-10: live RANGE (`g_e1`/`g_i1`) + deliver-wave coalesce.
Warm 1.80→1.58 s; ACC_TIME 292620→234. ✅ 400. Residual was first-dt
ensure_on_device (misread as t=1 TQ until re-timed).
...
kernels ≈ CN. Residual was host NET_RECEIVE full-mech SoA H→D (closed same day).

### Starting prompt — Traub product harness (closed 2026-08-04; archive)

Session closed 2026-08-04: product harness + ctest for Traub 1/10.
`test/external/traub/run_traub_native.sh`; refs no-gap 4474 / gap 7873;
`traub_native::neuron_gpu_native{,_gap}` (skip 77 if no model). Model stays
at `~/models/82894`. ctest Passed ~18 s / ~19 s. Docs updated.
...
Session closed 2026-08-04: Phase C min-present + flush wait coalesce + **deviceptr
for weights/live SoA** (root cause was per-launch present re-copyin: NMDA alone
~0.55 s device copy → 0). deliver-nrb ~2.07→~1.75; multi-warm ~13.5→~12.8–13.0.
4474 exact; 688@100 noise-only.

### Starting prompt — Traub residual ion SoA host-present (closed 2026-08-04; archive)

Session closed 2026-08-04: CN-style ion base[idx] with host present(base[:n],
idx[:m]) (RANGE shape). deviceptr of device base was illegal address; host
present green. Wall flat (~11.35–11.70 s; setup-rhs 2.18 flat). 4474 exact;
ringtest noise-only. Unit ACC ion present.

### Starting prompt — Traub residual nonvint / setup-rhs (closed; archive harness default)

Session closed 2026-08-04 as residual #16 wall-flat + product harness (ledger
GPU-P4-traub-harness). Density ~1.13× remains optional new residual only.

### Starting prompt — Traub residual host deliver-events (closed; archive)

Session closed 2026-08-04: sub-buckets proved residual is **deliver-nrb ~2.0 s**
not TQ (~0.4 s). Hygiene: no heap string in NetCon::deliver; empty flush skip;
step-scoped sorted token; lazy NRB ensure. Wall flat (~13.0–13.6 s).
4474 exact; 688@100 noise-only.

### Starting prompt — Traub residual PP CURRENT area SoA (closed; archive)

Session closed 2026-08-04: point-process CURRENT mfactor via `_d_area[node_id]`
(node area SoA deviceptr) not pdata dptr. Unit ExpSynAcc. Wall flat (~13.3–13.6 s).
4474 exact; 688@100 noise-only.
...
~1.5–1.6 s with MPS). Ringtest 2-rank 688 green. Residual was dentate 390 vs 400.
...
testcorenrn_kin_native green. Ringtest 688@100 + 2-rank MPI green.

### Starting prompt — dentate GC CadepK NaN / 400 spikes (closed; archive)

Session closed 2026-08-03: Eigen residual host `inst.global->` → CadepK NaN STATE
...
### Starting prompt — phases=0 ACC wait (closed; archive)

Session closed 2026-08-04: host-read D→H stream fence in
`sync_state_to_host_*` (before `update host`). Session E left CURRENT/STATE/JACOB
...
### Starting prompt — Phase C denser-spike remeasure (closed; archive)

Session closed 2026-08-04 on explor `local/gpu-p4-phase-c-remeasure`
(cherry-pick of Phase C onto tip): product 688@100 green; ExpSyn/Exp2Syn
slim net_buf; kernel avg ~10 µs. Ringtest multi-warm ~1.13–1.15 s flat vs
...
|--------|-------------------------|-------|
| `use_gap=0` | QUALIFIED A–F; **4474 exact** native↔CPU↔CN | Multi-warm ~**13.1–13.2 s** (post ion_cur cache); CN ~10 s (~1.3×) |
...
| Density ion_cur cache | per-type iontype; no mid-rhs wait / thrash | setup-rhs 3.16→2.19 s; wall ~14.3→13.2 s |
...
**Default next:** product bars closed (ringtest + dentate + Traub ctest). Optional
new density residual only with measured wall hypothesis, or other portfolio row.
Phase C re-parked. Traub identity + density #1–#16 + harness + 2026-08-06 re-smoke closed.

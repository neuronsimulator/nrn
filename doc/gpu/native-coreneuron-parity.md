...
**Tree:** `~/neuron/nrngpu`  
**Living tip (2026-09-10):** `local/gpu-native` (master merge `76ba78245` + host-NR SoA slim + NRB-fast). Campaign `2026-09-10-tip-psolve-matrix`: 8×4 psolve, identity vs CPU. Dentate nt1 1-rank: CN GPU warm ~**0.53 s** vs native ~**1.56 s** (≈**3.0×**; ✅ 400). **`H-dentate-nt1-deliver` closed:** deliver-tq **0.016 s** (not the 0.81 s); NRB-fast 3163/3163 NetCon. Residual is first-dt **`ensure_on_device` 0.68 s** (`H-dentate-nt1-device-ensure`). Traub no-gap native ~**1.21×** CN GPU; Traub gap native ✅ 7873 / CN 7867. Ring160 nt1 native **faster** than CN GPU on T1000.  
...
19. **Dentate nt1 exclusive 3.4× (attributed 2026-09-10; recoded same day):** 1-rank ×3 psolve, ✅ 400. L1+L2: STATE/CURRENT kernels **≈ CN**; NRB 0.009; gap 0.037. **`H-dentate-nt1-host-nr-soa` closed:** host NET_RECEIVE live RANGE + coalesce (292 620 → 234 H→D; warm **1.80 → 1.58 s**, ≈**3.0×** CN 0.53 s). **`H-dentate-nt1-deliver` closed:** deliver-tq **0.016 s** after SoA slim (3163 NetCon / 3922 SelfEvent); NRB-fast enqueue all NetCons. The hoc t=1 ~0.75 s is the **first dt** `ensure_on_device` **0.68 s** (`H-dentate-nt1-device-ensure`). Do not reopen density / ion SoA / net_buf / NSB / deliver-tq for this cell.
...
**Next:** Dentate nt1 first-dt `ensure_on_device` (`H-dentate-nt1-device-ensure`). Warm native ~**1.56 s** vs CN GPU ~**0.53 s** (≈**3.0×**); first step **0.68 s** (sorted-token upload). deliver-tq **0.016 s** (closed). Not density. Not 4-rank MPS. Not ion SoA / net_buf / NSB.

### Starting prompt — Dentate nt1 first-step device-ensure (next)

```text
Read ~/neuron/notes/PORTFOLIO.md (GPU-native + Devbench),
~/neuron/nrngpu/doc/gpu/dentate-nt1-attribution.md (L1+L2 + recode),
native-coreneuron-parity.md Status/Next, GROK-GPU-NATIVE.md, AGENTS.md.

Kind: feature. Cwd: ~/neuron/nrngpu @ local/gpu-native.
Hypothesis: H-dentate-nt1-device-ensure — first dt of each psolve is
  ~0.68 s in ensure_on_device / device_token upload (warm psolve ~1.56 s
  vs CN GPU ~0.53 s). deliver-tq closed at 0.016 s. Host-NR SoA closed.
  Identity ✅ 400.

This session: attribute the first-step upload (token teardown vs present
  rebuild vs copyin) and, if ≥ ~0.2 s with a wall hypothesis, recode so
  device mirrors survive stdinit/psolve. Re-measure 1-rank Dentate nt1
  3× psolve. Do not mix 4-rank MPS. Heap-free weight_index.
  Not deliver-tq. Not density. Not ion SoA / net_buf / NSB.

...
**Active branch:** `local/cpu-net-soa-heap-free` (no PR). Charter + two-epoch north star: [heap-free.md](heap-free.md).

**Steps 1–6 done** on that branch (`NetCon::weight_` removed). Remaining: zero-copy SoA `double*` (6b), then nocmodl **weight_index** receive (7) to drop TLS/FOR_NETCONS scratch. Structure change still clears the queue; edit between runs stays fully NEURON-flexible.

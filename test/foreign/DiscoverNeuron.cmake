# Discover a foreign (installed) NEURON via NRN_FOREIGN_PYTHON and
# probe_neuron.py.
#
# PATH and PYTHONPATH use ';' on Windows and ':' on Unix. Joining with ':' made
# shutil.which miss nrnivmodl.cmd in the venv Scripts directory.
#
# Sets (CACHE INTERNAL unless noted): NRN_FOREIGN_PYTHON              -
# interpreter used for discovery (user-facing CACHE) NRN_FOREIGN_NEURON_VERSION
...
# NRN_FOREIGN_MODLUNIT NRN_FOREIGN_MPIEXEC NRN_FOREIGN_FEATURE_<NAME>      -
# ON/OFF for known keys NRN_FOREIGN_PROBE_JSON          - path to last probe
# output file
...
set(NRN_FOREIGN_PYTHON
    ...
    CACHE
      FILEPATH
      "Python interpreter that can import the foreign NEURON install (venv wheel)"
)
...
if(NOT EXISTS "${NRN_FOREIGN_PYTHON}")
  message(
    FATAL_ERROR "NRN_FOREIGN_PYTHON does not exist: ${NRN_FOREIGN_PYTHON}")
endif()
...
# Prefer scripts next to NRN_FOREIGN_PYTHON (venv bin/) and optional
# NRN_FOREIGN_ROOT/bin. Drop ambient PYTHONPATH so a developer’s *other*
# build-tree install cannot shadow the foreign install. For a classic prefix,
# put ${ROOT}/lib/python on PYTHONPATH (NEURON’s default
...
if(NOT NRN_FOREIGN_ROOT STREQUAL "")
  if(NOT IS_DIRECTORY "${NRN_FOREIGN_ROOT}")
    message(
      FATAL_ERROR "NRN_FOREIGN_ROOT is not a directory: ${NRN_FOREIGN_ROOT}")
  endif()
...
# Optional override (native-separated), e.g. custom NRN_INSTALL_PYTHON_PREFIX
# parent.
set(NRN_FOREIGN_PYTHONPATH
    ""
    CACHE STRING
          "Optional PYTHONPATH entries for foreign discovery/tests (prepended)")
...
# Expose for ForeignTestHelpers / tests (FORCE so reconfigure with ROOT
# updates).
...
set(NRN_FOREIGN_SITE_PYTHONPATH
    ...
    CACHE INTERNAL
          "PYTHONPATH prefix for foreign neuron package (prefix installs)"
          FORCE)
...
execute_process(
  COMMAND
    ...
    "PATH=${_nrn_foreign_probe_path}${NRN_FOREIGN_ENV_SEP}$ENV{PATH}"
...
# CMake 3.19+ string(JSON). path is a semicolon list of members, e.g.
# "tools;nrniv"
...
_nrn_foreign_json_get(NRN_FOREIGN_NEURON_VERSION_FULL "${_probe_stdout}"
                      version_full)
...
if(NOT _features_error STREQUAL "")
  message(
    WARNING
      "Foreign probe could not read neuron.config.arguments: ${_features_error}\n"
...
if(NOT _missing_feature_keys STREQUAL "")
  message(
    STATUS
      "Foreign feature keys missing (treated as OFF): ${_missing_feature_keys}")
endif()
...
if(NRN_FOREIGN_NRNIV STREQUAL "")
  message(
    WARNING
      "Foreign nrniv not found on PATH; HOC smoke and some tests will be skipped"
  )
endif()
...
if(NRN_FOREIGN_FEATURE_NRN_ENABLE_MPI AND NRN_FOREIGN_MPIEXEC STREQUAL "")
  message(
    STATUS "Foreign NEURON has MPI, but mpiexec was not found on PATH; "
...
# If CoreNEURON is on but SHARED is unknown/missing, assume shared (wheels are
# shared).
if(NRN_FOREIGN_FEATURE_NRN_ENABLE_CORENEURON
   AND NOT NRN_FOREIGN_FEATURE_CORENRN_ENABLE_SHARED_KNOWN)
  message(
    STATUS
      "CORENRN_ENABLE_SHARED not reported by wheel; assuming ON (typical for wheels)"
  )
...
message(
  STATUS "Foreign MPI               : ${NRN_FOREIGN_FEATURE_NRN_ENABLE_MPI}")
message(
  STATUS
    "Foreign MPI dynamic       : ${NRN_FOREIGN_FEATURE_NRN_ENABLE_MPI_DYNAMIC}")
message(
  STATUS
    "Foreign CoreNEURON        : ${NRN_FOREIGN_FEATURE_NRN_ENABLE_CORENEURON}")
message(
  STATUS
    "Foreign CoreNEURON shared : ${NRN_FOREIGN_FEATURE_CORENRN_ENABLE_SHARED}")
message(
  STATUS "Foreign GPU               : ${NRN_FOREIGN_FEATURE_CORENRN_ENABLE_GPU}"
)

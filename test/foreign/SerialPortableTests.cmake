...
# test/foreign/CMakeLists.txt after ForeignTestHelpers. MPI / CoreNEURON groups
# live in MpiCoreNeuronTests.cmake (M4).
...
foreach(ext hoc py)
  ...
  foreach(hoc_script ${hoc_scripts})
    ...
    if(ext STREQUAL "py")
      # Match in-tree: plain python interpreter (not pytest, not special
      # -python).
      nrn_add_test(
        GROUP
        hoctests
...
if(NOT NRN_FOREIGN_NRNIV STREQUAL "")
  ...
  add_test(
    ...
    COMMAND
      ...
      -Dout_file=out.dat
      -Dref_file=${NRN_FOREIGN_SOURCE_ROOT}/test/ringtest/out.dat.ref
      -Dwork_dir=${_ring_dir} -P
      ${NRN_FOREIGN_SOURCE_ROOT}/cmake/RunHOCTest.cmake)
  set_tests_properties(foreign::ringtest
                       PROPERTIES LABELS "foreign;serial;ringtest" TIMEOUT 120)
  ...
  set_tests_properties(
    foreign::connect_dend PROPERTIES LABELS "foreign;serial;connect_dend"
                                     TIMEOUT 120)
endif()
...
if(NRN_ENABLE_RX3D AND _nrn_foreign_have_pytest)
  ...
  if(NOT EXISTS "${_rxd_testdata}/.git" AND NOT EXISTS
                                            "${_rxd_testdata}/README")
    # Try to populate the submodule once at configure time.
    find_package(Git QUIET)
    if(GIT_FOUND AND EXISTS "${NRN_FOREIGN_SOURCE_ROOT}/.gitmodules")
      execute_process(
        COMMAND "${GIT_EXECUTABLE}" -C "${NRN_FOREIGN_SOURCE_ROOT}" submodule
                update --init -- test/rxd/testdata

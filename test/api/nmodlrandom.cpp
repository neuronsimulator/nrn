...
int main() {
    ...
    Object* density_left = nrn_nmodlrandom_wrap(cable, 1.0 / 6.0, density_rng);
    Object* density_right = nrn_nmodlrandom_wrap(cable, 5.0 / 6.0, density_rng);
    ...
    density_left = nrn_nmodlrandom_wrap(cable, 1.0 / 6.0, density_rng);
    ...
    ok &= check(!nrn_nmodlrandom_wrap(nullptr, 0.5, density_rng), "null section rejected");
    ok &= check(!nrn_nmodlrandom_wrap(cable, 0.5, nullptr), "null density symbol rejected");
    ok &= check(!nrn_nmodlrandom_wrap(cable, 0.5, nrn_symbol("v")),
                "non-RANDOM density symbol rejected");
    ok &= check(!nrn_nmodlrandom_wrap(cable, -0.1, density_rng),
                "out-of-range density position rejected");
    ok &= check(!nrn_nmodlrandom_wrap(empty, 0.5, density_rng),
    ...
    Object* point_random = nrn_pntproc_nmodlrandom_wrap(point, point_rng);
    ...
    point_random = nrn_pntproc_nmodlrandom_wrap(point, point_rng);
    ...
    ok &= check(!nrn_pntproc_nmodlrandom_wrap(nullptr, point_rng), "null point process rejected");
    ok &= check(!nrn_pntproc_nmodlrandom_wrap(point, nullptr), "null point symbol rejected");
    ok &= check(!nrn_pntproc_nmodlrandom_wrap(vector, point_rng), "non-point object rejected");
    ok &= check(!nrn_pntproc_nmodlrandom_wrap(point, density_rng),
                "RANDOM symbol from another mechanism rejected");
    ok &= check(!nrn_pntproc_nmodlrandom_wrap(unlocated, nrn_method_symbol(unlocated, "rng")),

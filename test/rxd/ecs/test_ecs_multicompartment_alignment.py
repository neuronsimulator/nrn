...
def multicompartment_alignment(neuron_instance):
    ...
    #                                 the section
    fhs = rxd.Region([soma], name='fhs', nrn_region='o', 
                     geometry=rxd.Shell(1,1.2))
    
    # a border -- between FHS and ECS
    bor = rxd.Region([soma], name='bor', 
                     geometry=rxd.ScalableBorder(diam_scale=1.2))
    
    # the shared ECS
    ecs = rxd.Extracellular(-15, -15, -15, 15, 15, 15, dx=dx, tortuosity=1.6, 
    ...
    # seg.ki and seg.ko
    k = rxd.Species([cyt, fhs], d=Dk, name='k', charge=1,
                     initial=lambda nd: 140.0 if nd.region == cyt else 
                                        3.0)
    ...
    diffusion = rxd.MultiCompartmentReaction(k[fhs], kecs[ecs],

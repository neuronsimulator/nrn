def test_only_nodes(neuron_instance):
    ...
    try:
        nl = rxd.nodelist.NodeList([1,2,3,water.nodes[0]])   #create NodeList with non-nodes
        raise Exception("should not get here")
    except TypeError:
        ...
    ...
    try:
        nl = rxd.nodelist.NodeList([water.nodes[0], water.nodes[0]]) #create nodelist with nodes
    except TypeError:
        print("Nodelist should only contain nodes.")
        raise Exception("should not get here")

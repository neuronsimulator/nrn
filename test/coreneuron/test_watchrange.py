...
class Cell():
  def __init__(self, gid):
    ...
    self.syn = h.Bounce(self.soma(.5))
    pc.cell(gid, h.NetCon(self.soma(.5)._ref_v, None, sec=self.soma))
  ...
  def result(self):
    return (self.syn.n_high, self.syn.n_mid, self.syn.n_low, self.t1vec.c(), self.xvec.c(), self.rvec.c())
...
def test_watchrange():
  ...
  def runassert(mode):
    ...
    for i, cell in enumerate(cells):
      ...
      for j in range(3):
        if std[j] != result[j]:
          print("cell=%d %s:(%d %d) mode=%d" % (i, hml[j], std[j], result[j], mode))
          if not std[4].eq(result[4]):
            k = int(std[4].c().sub(result[4]).indwhere("!=", 0))
            print("first difference at %d (%g, %s, r=%g) vs (%g, %s, r=%g)" %(k,std[3][k],hml[int(std[4][k])],std[5][k],result[3][k],hml[int(result[4][k])],result[5][k]))
            for ik in range(k+1):
              print("  %d %g (%g, %s, r=%g) vs (%g, %s, r=%g)" %(ik, tvec[ik], std[3][ik], hml[int(std[4][ik])], std[5][ik], result[3][ik], hml[int(result[4][ik])], result[5][ik]))

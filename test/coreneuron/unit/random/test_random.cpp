...
TEST_CASE("random123 smoke test") {
    ...
    double ref[] = {0.724128, 0.507056, 0.446231, 0.331509, 0.84732, 0.0942423,
    0.5401,0.0496035,0.286901,0.777649,0.584096,0.468876,0.545774,0.584586,
    0.630361,0.875837,0.703636,0.853706,0.173552,0.917358};

    double res[NUM_TRIES];
    s = nrnran123_newstream(KEY_1, KEY_2);

    nrn_pragma_omp(target teams distribute parallel for simd map(tofrom: res[0:NUM_TRIES]) is_device_ptr(s) num_teams(1) thread_limit(1))

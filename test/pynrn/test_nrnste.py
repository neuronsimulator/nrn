...
def act(i, m, thresh, result):
  print("act %d t=%g (%g) v=%g thresh=%g" % (i, h.t, pc.t(0), m['s'](.5).v, thresh[0]))
...
def chk_result(sz, result, m):
  ...
  for x in result:
    ...
    i = x[2]
    vvec = m['vvec']
    print("vvec[%d]=%.18g th=%.18g vvec[%d]=%.18g" % (i-1, vvec[i-1], th, i, vvec[i]))
...
def model():
  ...
  ic.dur = 10
  ic.amp = .1
  vvec = h.Vector().record(s(.5)._ref_v, sec=s)

...
extern "C" void _netstim_reg_() {
    ...
    ivoc_help(
        "help ?1 NetStim "
        "/Users/kumbhar/workarena/repos/bbp/nrn/src/nrnoc/netstim.mod\n");
...
static int seed(_internalthreadargsprotocomma_ double);
...
static int seed(_internalthreadargsprotocomma_ double _lx) {
/*VERBATIM*/
#if !NRNBBCORE
    set_seed(_lx);
...
static double _hoc_seed(void* _vptr) {
    ...
    _r = 1.;
    seed(_threadargscomma_ * getarg(1));
    return (_r);
}
...
/*VERBATIM*/
    ...
    if (ifarg(3)) {
        ...
        r123state = nrnran123_newstream(static_cast<uint32_t>(*getarg(1)),
                                        static_cast<uint32_t>(*getarg(2)));
    }
#endif
    return 0;
...
static void bbcore_write(double* x, int* d, int* xx, int* offset, _threadargsproto_) {
    ...
    if (!_p_donotuse) {
        fprintf(stderr,
                "NetStim: cannot use the legacy scop_negexp generator for "
                "the random stream.\n");
        assert(0);
    }
    if (d) {
...
static void _net_receive(Point_process* _pnt, double* _args, double _lflag) {
    ...
    if (_tsav > t) {
        hoc_execerror(hoc_object_name(_pnt->ob),
                      ":Event arrived out of order. Must call "
                      "ParallelContext.set_maxstep AFTER "
...
static void register_nmodl_text_and_filename(int mech_type) {
    ...
    const char* nmodl_file_text =
        ...
        "\n"
        ": For backward compatibility, noiseFromRandom(hocRandom) can still be "
        "used\n"
        ": as well as the default low-quality scop_exprand generator.\n"
        ": However, CoreNEURON will not accept usage of the low-quality "
        "generator,\n"
        ": and, if noiseFromRandom is used to specify the random stream, that "
        "stream\n"
        ": must be using the Random123 generator.\n"
        "\n"
        ": The recommended idiom for specfication of the random stream is to "
        "use\n"
        ": noiseFromRandom123(id1, id2[, id3])\n"
        "\n"
        ": If any instance uses noiseFromRandom123, then no instance can use "
        "noiseFromRandom\n"
        ": and vice versa.\n"
        "\n"
        "NEURON	{ \n"
        ...
        "    RANGE noise\n"
        "    THREADSAFE : only true if every instance has its own distinct "
        "Random\n"
        ...
        "    number      = 10 <0,1e9>            : number of spikes (independent "
        "of noise)\n"
        "    start       = 50 (ms)               : start of first spike\n"
        "    noise       = 0 <0,1>               : amount of randomness (0.0 - "
        "1.0)\n"
        "}\n"
        "\n"
        "ASSIGNED {\n"
        ...
        "VERBATIM\n"
        "#if !NRNBBCORE\n"
        "/** If we're running in NEURON, specify the noise style for all "
        "instances.\n"
        " *  1 means noiseFromRandom was called when _ran_compat was previously "
        "0.\n"
        ...
        "        /* only this style initializes the stream on finitialize */\n"
        "        "
        "nrnran123_setseq(reinterpret_cast<nrnran123_State*>(_p_donotuse), 0, "
        "0);\n"
        "    }\n"
        "    ENDVERBATIM\n"
        "    on = 0 : off\n"
        ...
        "        } else {\n"
        "            _lerand = "
        "nrn_random_pick(reinterpret_cast<Rand*>(_p_donotuse));\n"
        "        }\n"
        "#else\n"
        "        _lerand = "
        "nrnran123_negexp(reinterpret_cast<nrnran123_State*>(_p_donotuse));\n"
        ...
        " {\n"
        "    if (_ran_compat == 2) {\n"
        "        fprintf(stderr, \"NetStim.noiseFromRandom123 was previously "
        "called\\n\");\n"
        "        assert(0);\n"
        "    }\n"
        "    _ran_compat = 1;\n"
        ...
        "#if !NRNBBCORE\n"
        "    if (_ran_compat == 1) {\n"
        "        fprintf(stderr, \"NetStim.noiseFromRandom was previously "
        "called\\n\");\n"
        "        assert(0);\n"
        "    }\n"
        "    _ran_compat = 2;\n"
        "    auto& r123state = "
        "reinterpret_cast<nrnran123_State*&>(_p_donotuse);\n"
        "    if (r123state) {\n"
        "        nrnran123_deletestream(r123state);\n"
        "        r123state = nullptr;\n"
        "    }\n"
        "    if (ifarg(3)) {\n"
        "        r123state = "
        "nrnran123_newstream3(static_cast<uint32_t>(*getarg(1)), "
        "static_cast<uint32_t>(*getarg(2)), static_cast<uint32_t>(*getarg(3)));\n"
        "    } else if (ifarg(2)) {\n"
        "        r123state = "
        "nrnran123_newstream(static_cast<uint32_t>(*getarg(1)), "
        "static_cast<uint32_t>(*getarg(2)));\n"
        "    }\n"
        "#endif\n"
        "ENDVERBATIM\n"
        ...
        "    /* error if using the legacy scop_exprand */\n"
        "    if (!_p_donotuse) {\n"
        "        fprintf(stderr, \"NetStim: cannot use the legacy scop_negexp "
        "generator for the "
        "random stream.\\n\");\n"
        "        assert(0);\n"
        "    }\n"
        ...
        "            /* error if not using Random123 generator */\n"
        "            if (!nrn_random_isran123(rand, di, di+1, di+2)) {\n"
        "                fprintf(stderr, \"NetStim: Random123 generator is "
        "required\\n\");\n"
        "                assert(0);\n"
        "            }\n"
        "            nrn_random123_getseq(rand, di+3, &which);\n"
        ...
        "_threadargsproto_) {\n"
        "    if (!noise) { return; }\n"
        "    /* Generally, CoreNEURON, in the context of psolve, begins with an "
        "empty\n"
        "     * model, so this call takes place in the context of a freshly "
        "created\n"
        "     * instance and _p_donotuse is not NULL.\n"
        "     * However, this function is also now called from NEURON at the end "
        "of\n"
        "     * coreneuron psolve in order to transfer back the nrnran123 "
        "sequence state.\n"
        "     * That allows continuation with a subsequent psolve within NEURON "
        "or\n"
        "     * properly transfer back to CoreNEURON if we continue the psolve "
        "there.\n"
        "     * So now, extra logic is needed for this call to work in a NEURON "
        "context.\n"
        "     */\n"
        "    uint32_t* di = reinterpret_cast<uint32_t*>(d) + *offset;\n"
        "#if NRNBBCORE\n"
        ...
        "neuron receiving various types of presynaptic trains.  This is a "
        "\"fake\"\n"
        ...
        "generators can stimulate other generators to create complicated "
        "bursting\n"
        ...
        "that time it ignores any positive weight events. If, in an on!=0 "
        "state,\n"
        "the stimulator receives a negative weight event, the stimulator will\n"
        "change to the on==0 state. In the on==0 state, it will ignore any "
        "ariving\n"
        "net_send events. A change to the on==1 state immediately fires the "
        "first spike of\n"
        "its sequence.\n"
        "\n"
        "ENDCOMMENT\n"

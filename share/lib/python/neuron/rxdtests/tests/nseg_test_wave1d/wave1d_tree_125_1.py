...
recdt = 100
datacol=0
data= numpy.zeros( (sec.nseg*nsubseg*3, numpy.ceil(tstop/recdt) ) )
...
def mydraw (data=data,vmin=[0,0,0],vmax=[0.002,0.2, 0.011],startt=0,endt=tstop,\
            miny=0,maxy=int(sec.L*3),keys=["cytca"],titles=["ca[cyt] (mM)","ip3[cyt] (mM)"],\
            interp='bilinear'):
  dat = data
  i=0
...
#  pyplot.imshow(dat, vmin=vmin[i], vmax=vmax[i],aspect='auto',extent=(0,tstop/1e3,0,int(sec.L*3)), origin='lower',interpolation=interp)
  pyplot.imshow(dat, vmin=mymin, vmax=mymax,aspect='auto',extent=(0,tstop/1e3,0,int(sec.L*3)), origin='lower',interpolation=interp)
...
#pyplot.savefig('./gif/tsttree_voltage_over_time'+str(sec.nseg)+"_"+str(nsubseg)+'.png')
pyplot.figure()
mydraw()
numpy.save("treeca_2d"+str(rxd.options.subseg_interpolation)+str(rxd.options.subseg_averaging)+"_nrxd"+str(sec.nseg)+"_"+str(nsubseg), data)
#pyplot.savefig('./gif/tsttree_ca_2d'+str(sec.nseg)+"_"+str(nsubseg)+'.png')

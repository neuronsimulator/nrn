...
recdt = 100
datacol=0
data= numpy.zeros( (sec.nseg*nsubseg, numpy.ceil(tstop/recdt) ) )


# record cytoplasmic ca concentration
def dorecord ():
  global datacol
  dorec(data, ca[cyt].nodes(sec))
  datacol = datacol+1


def save():
    print 't = %g, cai_mid = %g' % (h.t, sec.cai), ca[cyt].nodes[0].value
    s.flush()
...
def mydraw (data=data,vmin=[0,0,0],vmax=[0.002,0.2, 0.011],startt=0,endt=tstop,\
  ...
  print dend.L, sec.L, "ggg"
  pyplot.imshow(dat, vmin=vmin[i], vmax=vmax[i],aspect='auto',extent=(0,tstop/1e3,0,int(sec.L)), origin='lower',interpolation=interp)
...
print len(v1.as_numpy()),len(ca1.as_numpy()),len(v2.as_numpy()),len(ca2.as_numpy())
...
#pyplot.savefig('./gif/voltage_over_time'+str(sec.nseg)+"_"+str(nsubseg)+'.png')
pyplot.figure()
mydraw()
numpy.save("newca_2d"+str(rxd.options.subseg_interpolation)+str(rxd.options.subseg_averaging)+"_"+str(sec.nseg)+"_"+str(nsubseg), data)
#pyplot.savefig('./gif/ca_2d'+str(sec.nseg)+"_"+str(nsubseg)+'.png')

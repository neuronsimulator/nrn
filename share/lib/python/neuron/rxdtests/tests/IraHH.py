...
na = rxd.Species([cyt, mem, ecs], name='na', d=1, charge=1, initial=concentration(15.5, 161.5))
k = rxd.Species([cyt, mem, ecs], name='k', d=1, charge=1, initial=concentration(142.5, 7.8))
cl = rxd.Species([cyt, mem, ecs], name='cl', d=1, charge=-1, initial=concentration(6.0, 130))
...
#sodium (Na) current
na_current = rxd.MultiCompartmentReaction(nai, nao, gna*(v - ena), mass_action=False, membrane=mem, membrane_flux=True)

#potassium (K) current
k_current = rxd.MultiCompartmentReaction(ki, ko, gk*(v - ek), mass_action=False, membrane=mem, membrane_flux=True)

#chlorine (Cl) current
cl_current = rxd.MultiCompartmentReaction(cli, clo, gclbar_l * (v - ecl), mass_action=False, membrane=mem, membrane_flux=True)

#nkcc1 (Na+/K+/2Cl- cotransporter)
nkcc1_current = rxd.MultiCompartmentReaction(2*clo + ko + nao, 2*cli + ki + nai, nkcc1 *volume_scale, mass_action=False, membrane=mem, membrane_flux=True)

#kcc2 (K+/Cl- cotransporter)
kcc2_current = rxd.MultiCompartmentReaction(clo + ko, cli + ki, kcc2 *volume_scale, membrane=mem, mass_action=False, membrane_flux=True)

#sodium leak
na_leak = rxd.MultiCompartmentReaction(nai, nao, gnabar_l*(v - ena), mass_action=False, membrane=mem, membrane_flux=True)

...
class middle_man:
	...
	def add_reaction(self,name,reversible,reactants,products,kinetic_law,modifiers):
		self.reactions.append(reaction(name,reversible,reactants,products,kinetic_law,modifiers))
	...
	def create_xml(self):
		sbml = ET.Element("sbml",level="2",version="3",xmlns="http://www.sbml.org/sbml/level2/version3")
		model = ET.SubElement(sbml,"model", name=self.name)
		if len(self.unit_defs) > 0: unitdefinitions = ET.SubElement(model,"listOfUnitDefinitions")
		for key,value in self.unit_defs.items():
			unitDefinition = ET.SubElement(unitdefinitions,"unitDefinition", id=value.name, name=value.name)
			listOfUnits = ET.SubElement(unitDefinition,"listOfUnits")
			for j in value.units:
				ET.SubElement(listOfUnits,"unit", kind=j[0] ,  exponent=str(j[1]) ,scale=str(j[2]))




		if len(self.compartments) > 0: listOfCompartments = ET.SubElement(model,"listOfCompartments")
		for key,value in self.compartments.items():
			ET.SubElement(listOfCompartments,"compartment",id=value.name,name=value.name, size=str(value.size))




		if len(self.species) > 0: listOfSpecies = ET.SubElement(model,"listOfSpecies")
		for key,value in self.species.items():
			ET.SubElement(listOfSpecies,"species",compartment=value.compartment,id=key, name=value.name, initialConcentration=value.initial_amount)




		if len(self.parameters) > 0: listOfParameters = ET.SubElement(model,"listOfParameters")
		for para,value in self.parameters.items():
			ET.SubElement(listOfParameters,"parameter", id=para,value=str(value.value))




		if len(self.reactions) > 0: listOfReactions = ET.SubElement(model,"listOfReactions")
		for react in self.reactions:
			current_reaction = ET.SubElement(listOfReactions,"reaction",id=react.name, name=react.name, reversible= str(react.reversible).lower())
			if len(react.reactants) > 0: listOfReactants = ET.SubElement(current_reaction,"listOfReactants")
			for i in react.reactants:
				ET.SubElement(listOfReactants,"speciesReference",species=i[0], stoichiometry=str(i[1]))

			if len(react.products) > 0: listOfProducts = ET.SubElement(current_reaction,"listOfProducts")
			for i in react.products:
				ET.SubElement(listOfProducts,"speciesReference",species=i[0], stoichiometry=str(i[1]))

			if len(react.modifiers) > 0: listOfModifiers = ET.SubElement(current_reaction,"listOfModifiers")
...
def sbml(segment,filename=None,model_name = None,pretty=True):
	...
	for i in species:
		if isinstance(i,crxd.Parameter):
	...
	for i in crxd.rxd._all_reactions: 
		if isinstance(i,crxd.MultiCompartmentReaction): RxDException("Cannot export MultiCompartmentReactions")


	#MIGHT BE A PROBLEM WHEN MULTI COMPARTMENT REACTIONS
	reactions = [r() for r in crxd.rxd._all_reactions if (any([((reg in r()._active_regions) or (reg is None)) for reg in r()._regions])) and isinstance(r(),crxd.Reaction)]
	
	rates = [r() for r in crxd.rxd._all_reactions if (any([((reg in r()._active_regions) or (reg is None)) for reg in r()._regions])) and isinstance(r(),crxd.Rate)]
	...
	for react in reactions:
		...
		reacting_regions = []
		if react._regions == [None]: reacting_regions = regions
		else: reacting_regions = react._regions
		for reg in reacting_regions:
			reactants = [[key.name + "_" + reg.name,value] for key,value in zip(list(lhs.keys()),list(lhs.values()))]
			products = [[key.name + "_" + reg.name,value] for key,value in zip(list(rhs.keys()),list(rhs.values()))]
			...
			recursive_search(react._rate_arithmeticed,region_multiple,parameters_in_react,reg.name)
			listOfParameters = ET.Element("listOfParameters")


			for i in parameters_in_react:
				if i[0] in output.parameters: continue
				ET.SubElement(listOfParameters,"parameter", id=i[0],name=i[1],value=str(i[2]))


			if list(listOfParameters): kineticLaw.append(listOfParameters)
			output.add_reaction("Reaction_" + str(num),reversible = (react._dir == "<>"),products = products,reactants= reactants,kinetic_law=kineticLaw,modifiers=[])
			num += 1
	...
	for rate in rates:
		list_regions = []
		if rate._regions == [None]: list_regions = regions
		else: list_regions = rate._regions
		for r in list_regions:
			...
			reactants = []
			modifiers = [s().name + "_" + r.name for s in rate._involved_species if isinstance(s(),crxd.Parameter) == False and rate._species().name != s().name]
			kineticLaw = ET.Element("kineticLaw")
			math = ET.SubElement(kineticLaw,"math",xmlns="http://www.w3.org/1998/Math/MathML")
			region_multiple = ET.SubElement(math,"apply")
			ET.SubElement(region_multiple,"times")
			...
			output.add_reaction("Rate_" + str(num),reversible = True,products=products,reactants=reactants,kinetic_law=kineticLaw,modifiers=modifiers)
			num += 1
...
def recursive_search(arth_obj,kineticLaw,parameters,compartment_name):
    ...
    for item, count in zip(list(arth_obj._items.keys()), list(arth_obj._items.values())):
        if count:
            if isinstance(item, crxd.Parameter):
                ...
                items.append(temp)
                counts.append(count)
            elif isinstance(item, crxd.species._SpeciesMathable):
                temp = ET.Element("ci")
                if compartment_name == "": temp.text = item.name
                else: temp.text = item.name + "_" + compartment_name
                items.append(temp)
                counts.append(count)
...
def determine_type(obj,parameters,comp_name):
    ...
    if isinstance(obj, crxd.rxdmath._Product):
    ...
    elif isinstance(obj, crxd.rxdmath._Quotient):
        ET.SubElement(tree,"divide")
        recursive_search(obj._a,tree,parameters,comp_name)#arithmeticed
        recursive_search(obj._b,tree,parameters,comp_name)#arithmeticed
    elif isinstance(obj, crxd.rxdmath._Arithmeticed):
        recursive_search(obj,tree,parameters,comp_name)
    elif isinstance(obj, crxd.rxdmath._Function):
        if function_names[obj._fname] == "exception":
            ...
            recursive_search(obj._obj,tree,parameters,comp_name)
    elif isinstance(obj, crxd.rxdmath._Function2):
        if function_names[obj._fname] == "exception":
            if obj._fname == "atan2":

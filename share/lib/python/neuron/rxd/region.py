...
class Extracellular:
    ...
    def __init__(
        self, xlo, ylo, zlo, xhi, yhi, zhi, dx, volume_fraction=1, tortuosity=1
    ):
        from . import options
        ...
        if numpy.isscalar(volume_fraction):
            ...
            if alpha.shape != (self._nx, self._ny, self._nz):
                ...
                self.alpha = self._alpha.as_numpy().reshape(
                    self._nx, self._ny, self._nz
                )
        ...
        if numpy.isscalar(tortuosity):
            ...
            for i in range(self._nx):
                for j in range(self._ny):
                    for k in range(self._nz):
                        self.tortuosity[i, j, k] = tortuosity(
                            self._xlo + i * self._dx[0],
                            self._ylo + j * self._dx[1],
                            self._zlo + k * self._dx[2],
                        )
            ...
            if tortuosity.shape != (self._nx, self._ny, self._nz):
                raise RxDException(
                    "tortuosity must be a scalar or an array the same size as the grid: {0}x{1}x{2}".format(
                        self._nx, self._ny, self._nz
                    )
                )

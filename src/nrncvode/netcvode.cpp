...
declareTable(MaxStateTable, void*, MaxStateItem*)
    implementTable(MaxStateTable, void*, MaxStateItem*) declarePtrList(PreSynList, PreSyn)
        implementPtrList(PreSynList, PreSyn) declarePtrList(HTListList, HTList)
            implementPtrList(HTListList, HTList) declarePtrList(WatchList, WatchCondition)
                        ...
                        implementTable(PreSynTable, double*, PreSyn*)
                            declarePool(SelfEventPool, SelfEvent)
                                implementPool(SelfEventPool, SelfEvent)
                                    declarePtrList(TQList, TQItem) implementPtrList(TQList, TQItem)
...
declareNrnHash(SelfEventPPTable, long, Point_process*)
    implementNrnHash(SelfEventPPTable, long, Point_process*) SelfEventPPTable* SelfEvent::sepp_;
    ...
    if (src_) {
        for (int i = 0; i < src_->dil_.count(); ++i) {
            if (src_->dil_.item(i) == this) {
                src_->dil_.remove(i);
                if (src_->dil_.count() == 0 && src_->tvec_ == nil && src_->idvec_ == nil) {
#if 1 || NRNMPI

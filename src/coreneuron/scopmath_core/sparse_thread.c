...
static void prmat(SparseObj* so) {
  ...
  IGNORE(printf("\n        "));
  for (i = 1; i <= so->neqn; i++) IGNORE(printf("%1d", i % 10));
  IGNORE(printf("\n\n"));
  for (i = 1; i <= so->neqn; i++) {
    IGNORE(printf("%3d %3d ", so->diag[i]->row, i));
    j = 0;
    for (el = so->rowst[i]; el; el = el->c_right) {
      for (j++; j < so->varord[el->col]; j++) IGNORE(printf(" "));
      IGNORE(printf("*"));
    }
    IGNORE(printf("\n"));
  }
...
static void spar_minorder(
    SparseObj* so) { /* Minimum ordering algorithm to determine the order
...
void _nrn_destroy_sparseobj_thread(SparseObj* so) {
  ...
  if (so->varord) Free(so->varord);
  if (so->rhs) Free(so->rhs);
  if (so->coef_list) Free(so->coef_list);
  if (so->roworder) {
    for (i = 1; i <= so->nroworder; ++i) {
      Free(so->roworder[i]);
    }

...
CoreNEURON is a compute engine for the [NEURON](https://www.neuronsimulator.org/) simulator optimised for both memory usage and computational speed. Its goal is to simulate large cell networks with small memory footprint and optimal performance.
...
At the end of the simulation CoreNEURON transfers by default : spikes, voltages, state variables, NetCon weights, all Vector.record, and most GUI trajectories to NEURON. These variables can be recorded using regular NEURON API (e.g. [Vector.record](https://www.neuronsimulator.org/en/latest/progref/programming/math/vector.html#Vector.record) or [spike_record](https://www.neuronsimulator.org/en/latest/progref/modelspec/programmatic/network/parcon.html#ParallelContext.spike_record)).

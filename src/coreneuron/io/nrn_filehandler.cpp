...
void FileHandler::close() {
    ...
    // Delete source file if requested and read-mode is enabled
    const auto& env = getenv("CORENEURON_SHM_DELETESOURCE");
    if (env != NULL && std::string(env) == "ON" && current_mode == std::ios::in &&
        std::regex_match(current_filename, std::regex("^/dev/shm/.*_[1-3].dat$"))) {

...
void Phase2::transform_int_data(int elem0, int nodecount, int* pdata, int i, int n_node_) {
    ...
    for (auto tml = nt.tml; tml; tml = tml->next) {
        ...
        if (!corenrn.get_is_artificial()[type]) {
            ...
            for (int i = 0; i < szdp; ++i) {
                ...
                switch (s) {
                case -1:  // area
                    transform_int_data(nt._actual_area - nt._data, cnt, pdata, i, nt.end);
                    break;
                case -9:  // diam
                    transform_int_data(nt._actual_diam - nt._data, cnt, pdata, i, nt.end);
                    break;
                case -5:  // pointer assumes a pointer to membrane voltage
                    ...
                    /**
                    transform_int_data(
                        nt._actual_v - nt._data, cnt, pdata, i, nt.end);
                     **/
                    break;
                default:
                    if (s >= 0 && s < 1000) {  // ion
...
void Phase2::get_info_from_bbcore(NrnThread& nt,
    ...
    for (int i = 0; i < n_mech; ++i) {
        ...
        for (int j = 0; j < cntml; ++j) {
            ...
            double* d = ml->data;
            Datum* pd = ml->pdata;
            d += nrn_i_layout(jp, cntml, 0);
            pd += nrn_i_layout(jp, cntml, 0);
    ...
    for (auto tml = nt.tml; tml; tml = tml->next, ++itml) {
        ...
        if (corenrn.get_pnt_map()[type] > 0) {  // POINT_PROCESS mechanism including acell
            ...
            for (int i = 0; i < cnt; ++i) {
                ...
                pp->_type = type;
                pp->_i_instance = i;
                nt._vdata[ml->pdata[nrn_i_layout(i, cnt, 1)]] = pp;
                pp->_tid = nt.id;
            }
        }
        ...
        if (r.size()) {
            ...
            for (auto index: indices) {
                ...
                for (int i = 0; i < n; ++i) {
                    ...
                    ix += 5;
                    int ipd = ml->pdata[nrn_i_layout(i, cnt, index)];

...
template <typename T, typename M>
T* MutexedPool<T, M>::allocate(std::size_t n) {
#if NRN_ENABLE_THREADS
    std::lock_guard<M> l(mut_);
#endif
    if (n != 1) {
        throw std::runtime_error("MutexedPool allocator can only allocate one object at a time");
    }
...
template <typename T, typename M>
void MutexedPool<T, M>::deallocate(T* item, std::size_t) {
#if NRN_ENABLE_THREADS
    std::lock_guard<M> l(mut_);
#endif
    --nget_;
...
template <typename T, typename M>
bool MutexedPool<T, M>::is_valid_ptr(const T* p) const {

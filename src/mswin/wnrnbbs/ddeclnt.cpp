...
void nrnbbs_notify(const char* name, NrnBBSCallback f) {
    ...
    hData = DdeClientTransaction(NULL, 0, hConv, hszItem, wFmt, XTYP_ADVSTART, 1000, &dwResult);
    if (!hData) {
...
BOOL nrnbbs_connect() {
    if (!idInst) {
        ...
        if (DdeInitialize((LPDWORD) &idInst, (PFNCALLBACK) lpDdeProc, APPCMD_CLIENTONLY, 0L)) {
    ...
    while (hConv == (HCONV) NULL) {
        hConv = DdeConnect(idInst, hszService, hszUser, (PCONVCONTEXT) NULL);
        if (hConv == (HCONV) NULL) {
...
static void post(HCONV hc, const char* name) {
    if (hc != (HCONV) NULL) {
        ...
        hData = DdeCreateDataHandle(
            idInst, (LPBYTE) szDDEString, sizeof(szDDEString), 0L, hszItem, wFmt, 0);

        if (hData != (HDDEDATA) NULL) {
            hData = DdeClientTransaction(
                (LPBYTE) hData, -1, hc, hszItem, wFmt, XTYP_POKE, 1000, &dwResult);
        } else {
...
static BOOL take(HCONV hc, const char* name, BOOL look) {
    if (hc != (HCONV) NULL) {
        ...
        hData = DdeClientTransaction(NULL, 0, hc, hszItem, wFmt, XTYP_REQUEST, 1000, &dwResult);

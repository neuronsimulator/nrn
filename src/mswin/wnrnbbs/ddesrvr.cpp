...
LRESULT CALLBACK _export MainWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE:
        ...
        if (DdeInitialize((LPDWORD) &idInst, (PFNCALLBACK) lpDdeProc, APPCLASS_STANDARD, 0L)) {
            ...
            hConvApp = DdeConnect(idInst, hszService, 0L, (PCONVCONTEXT) NULL);
            if (hConvApp) {
    ...
    case WM_PAINT:
        ...
        for (i = 0; i < cTotalLines; i++) {
            ...
            TextOut(hDC, 0, y, (LPSTR) (szScreenText[j]), lstrlen(szScreenText[j]));
            y = y + cyChar;
        }
...
HDDEDATA EXPENTRY _export DDECallback(WORD wType,
    ...
    case XTYP_REQUEST: /* hsz1 topic ; hsz2 item */
        ...
        hData = DdeCreateDataHandle(
            idInst, (LPBYTE) szDDEString, sizeof(szDDEString), 0L, hsz1, wFmt, 0);

...
Section "Start Menu Shortcuts"
  ...
  CreateShortCut "$SMPROGRAMS\${NEURON}\NEURON Demo.lnk" "$INSTDIR\bin\neuron.exe" "-dll $INSTDIR\demo\release\nrnmech.dll demo.hoc" "$INSTDIR\bin\nrniv10.ico" 0
  SetOutPath "$INSTDIR"
  CreateShortCut "$SMPROGRAMS\${NEURON}\rxvt sh.lnk" "$INSTDIR\bin\rxvt.exe" "-fn 16 -fg black -bg white -sl 1000 -e $INSTDIR\bin\bash --rcfile $INSTDIR\lib\bshstart.sh -i" "$INSTDIR\bin\rxvt.exe" 0
...
Section "Desktop ${NEURON} folder with Shortcuts"
  ...
  SetOutPath "$INSTDIR"
  CreateShortCut "$DESKTOP\${NEURON}\rxvt sh.lnk" "$INSTDIR\bin\rxvt.exe" "-fn 16 -fg black -bg white -sl 1000 -e $INSTDIR\bin\bash --rcfile $INSTDIR\lib\bshstart.sh -i" "$INSTDIR\bin\rxvt.exe" 0

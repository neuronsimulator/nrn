...
static int EGApalette[16][3] = {{0, 0, 0},
                                {0, 0, 128},
                                {0, 128, 0},
                                {0, 128, 128},
                                {128, 0, 0},
...
/*****************************/
{
    ...
    fp = fopen(fname, "rb");
    if (!fp)
        return (gifError(pinfo, "can't open file"));
    ...
    if (strncmp((char*) dataptr, id87, (size_t) 6) == 0)
        ...
        return (gifError(pinfo, "not a GIF file"));
    ...
    if (aspect) {
        if (!gif89)
            return (gifError(pinfo, "corrupt GIF file (screen descriptor)"));
        else
    ...
    while (1) {
        ...
        if (block == EXTENSION) { /* parse extension blocks */
            ...
            if (fn == 'R') { /* GIF87 aspect extension */
                ...
                i = 12;
                for (; i < sbsize; i++)
                    NEXTBYTEa; /* read rest of first subblock */
            ...
            if (DEBUG)
                fprintf(stderr, "  at start: offset=%ld\n", dataptr - RawGIF);
            ...
            if (gotimage) { /* just skip over remaining images */
                ...
                /* skip image header */
                NEXTBYTEa;
                NEXTBYTEa; /* left position */
                NEXTBYTEa;
                NEXTBYTEa; /* top position */
                NEXTBYTEa;
                NEXTBYTEa; /* width */
                NEXTBYTEa;
                NEXTBYTEa;       /* height */
            ...
            else if (readImage(pinfo))
                gotimage = 1;
            if (DEBUG)
                fprintf(stderr, "  at end:   dataptr=%ld\n", dataptr - RawGIF);
        }
...
static int readImage(PICINFO* pinfo) {
    ...
    if (!pinfo->raster)
        return (gifError(pinfo, "couldn't malloc 'pic8'"));
...
static void doInterlace(int Index) {
    if (YC < Height)
        pinfo_->raster->poke(
            XC, Height - YC - 1, Red[Index] / 256., Green[Index] / 256., Blue[Index] / 256., 1.);

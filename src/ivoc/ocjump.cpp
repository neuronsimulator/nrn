...
#if HAVE_IV
bool Oc::valid_expr(Symbol *s) { return OcJump::execute(s->u.u_proc->defn.in); }
...
struct saved_state {
  ...
  void restore() {
    ...
    oc_restore_code(&c1, &c2, c3, &c4, &c5, &c6, &c7, &c8, c9, &c10, &c11,
                    &c12);
...
ObjectContext::~ObjectContext() { oc_restore_hoc_oop(&a1, &a2, &a4, &a5); }

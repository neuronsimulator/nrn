...
class CodegenNeuronAccVisitor: public CodegenNeuronCppVisitor {
  ...
  private:
    ...
    /**
     * Phase C: min-present net_buf_receive (live SoA columns only, drop dead
     * setup). Safe when NET_RECEIVE has no VERBATIM, no nested MOD PROCEDURE/
     * FUNCTION calls, and no POINTER/RANDOM. net_send/event keep nsb present.
     */

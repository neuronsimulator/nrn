 ...
 *
 * The goal is to replace statements with \ref solutions in place. In this way we can allow (to
...
class SympyReplaceSolutionsVisitor: public AstVisitor {
  ...
  private:
     ...
     * value). Since there are \ref pre_solve_statements_ and \ref tmp_statements; it is in general

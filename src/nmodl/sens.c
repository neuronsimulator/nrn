...
void sensparm(qparm) Item* qparm;
{
    if (!parmlist)
        parmlist = newlist();
...
void add_sens_statelist(s) Symbol* s;
{
    if (!statelist)
        statelist = newlist();
...
void sensmassage(type, qfun, fn) int type, fn;
Item* qfun;
{
    ...
    */
    ...
    ITERATE(q, parmlist) {
        if (type == DERIVATIVE) {
            Sprintf(buf,
                    "error=trajecsens(%d, _slist%d, _dlist%d,\
_p, &%s, %s, %s, %d, _slist%d+%d, _dlist%d+%d);\n if(error){abort_run(error);}\n",
            ...
            Sprintf(buf,
                    "error=steadysens(%d, _slist%d, _p, &%s, %s,\
 _dlist%d, %d, _slist%d+%d);\n if(error){abort_run(error);}\n",
            ...
            Sprintf(buf,
                    "error=linearsens(%d, _slist%d, _p, &%s, %s,\
...
void sens_nonlin_out(q, fun) Item* q;
Symbol* fun;
{

...
void c_out_vectorize(const char* prefix) {
    ...
    /* standard modl EQUATION without solve computes current */
    if (!conductance_) {
        P("\nstatic double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* "
          "_nt, double _v){double _current=0.;v=_v;");
#if CVODE
        if (cvode_nrn_current_solve_) {
            fprintf(fcout,
...
void vectorize_substitute(q, str) Item* q;
char* str;
{

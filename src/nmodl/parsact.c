...
void explicit_decl(level, q) int level;
Item* q;
{
...
void parm_array_install(n, num, units, limits, index) Symbol* n;
...
void parminstall(n, num, units, limits) Symbol* n;
char *num, *units, *limits;
{
    char buf[NRN_BUFSIZE];
...
*/
Symbol *ifnew_parminstall(name, num, units, limits) char *name, *num, *units, *limits;
{
    Symbol* s;
...
void steppedinstall(n, q1, q2, units) Symbol* n;
Item *q1, *q2;
char* units;
{
    int i;
...
void indepinstall(n, from, to, with, qstart, units, scop) Symbol* n;
 ...
 * parout.c.
 */
void depinstall(type, n, index, from, to, units, qs, makeconst, abstol) int type, index, makeconst;
Symbol* n;
...
void statdefault(n, index, units, qs, makeconst) Symbol* n;
int index, makeconst;
char* units;
Item* qs;
{
...
void queue_stmt(q1, q2) Item *q1, *q2;
{
    Symbol* s;
    static int first = 1;
...
void add_nrnthread_arg(q) Item* q;
...
void table_massage(tablist, qtype, qname, arglist) List *tablist, *arglist;
Item *qtype, *qname;
{
    Symbol *fsym, *s, *arg = 0;
    char* fname;
    ...
    Sprintf(buf,
            "static void _check_%s(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {\n",
            fname);
    vectorize_substitute(q, buf);
...
#if VECTORIZE
    Sprintf(buf,
            "%s(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double %s) {",
            fname,
            arg->name);
    vectorize_substitute(procfunc->prev, buf);
#endif
...
#if VECTORIZE
    Sprintf(buf,
            "_n_%s(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double %s){",
            fname,
            arg->name);
    vectorize_substitute(procfunc->prev, buf);
#endif
...
#if VECTORIZE
/* ARGSUSED */
void vectorize_use_func(qname, qpar1, qexpr, qpar2, blocktype) Item *qname, *qpar1, *qexpr, *qpar2;
int blocktype;
{
    Item* q;
...
int blocktype;
{
    ...
    lappendstr(procfunc, buf);
    vectorize_substitute(lappendstr(procfunc, ""),
                         "\tdouble* _p; Datum* _ppvar; Datum* _thread; _NrnThread* "

...
void massagereaction(qREACTION, qREACT1, qlpar, qcomma, qrpar) Item *qREACTION, *qREACT1, *qlpar,
    *qcomma, *qrpar;
{
    Reaction* r1;
...
void flux(qREACTION, qdir, qlast) Item *qREACTION, *qdir, *qlast;
{
    Reaction* r1;
...
void kinetic_intmethod(fun, meth) Symbol* fun;
char* meth;
{
    /*derivative form*/
    Reaction* r1;
...
static void genderivterms(r, type, n) Reaction* r;
...
void genfluxterm(r, type, n) Reaction* r;
int type;
int n;
{
    Symbol* s;
...
void kinetic_implicit(fun, dt, mname) Symbol* fun;
...
static int genconservterms(eqnum, r, fn, rlst) Reaction* r;
...
static int number_states(fun, prlst, pclst) Symbol* fun;
Rlist **prlst, **pclst;
{
...
static void kinlist(fun, rlst) Symbol* fun;
Rlist* rlst;
{
    int i;
    Symbol* s;
...
if (vectorize){
	...
	vectorize_substitute(qv, buf);
            ...
            Sprintf(buf, "_slist%d[%d] = &(%s) - _p;", fun->u.i, s->varnum, s->name);
            qv = lappendstr(initlist, buf);
#if 0 && VECTORIZE
if (vectorize){
...
void check_block(standard, actual, mes) int standard, actual;
char* mes;
{
    if (standard != actual) {
...
*/
void massagecompart(qexp, qb1, qb2, indx) Item *qexp, *qb1, *qb2;
Symbol* indx;
{
    Item *q, *qs;
...
void massageldifus(qexp, qb1, qb2, indx) Item *qexp, *qb1, *qb2;
Symbol* indx;
{
    Item *q, *qs, *q1;
    Symbol *s, *s2;
...
void kin_vect1(q1, q2, q4) Item *q1, *q2, *q4;
{
    if (!kvect) {
        kvect = newlist();
    }
...
void kin_vect3(q1, q2, q4) Item *q1, *q2, *q4;
{
    Symbol* fun;
...
void see_astmt(q1, q2) Item *q1, *q2;
{
#if 0 && VECTORIZE
	Item* q;
	if (vectorize) {
...
void cvode_kinetic(qsol, fun, numeqn, listnum) Item* qsol;
Symbol* fun;
int numeqn, listnum;
{
#if 1
    ...
    */
    ...
    sprintf(buf,
            "static int _ode_spec%d(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) "
            "{int _reset=0;{\n",
            fun->u.i);
    ...
    sprintf(buf,
            ...
            "_thread, _NrnThread* _nt) {int _reset=0;{\n",
            fun->u.i);
    vectorize_substitute(procfunc->prev, buf);
    cvode_flag = 1;
...
void single_channel(qsol, fun, numeqn, listnum) Item* qsol;
Symbol* fun;
int numeqn, listnum;
{
    Rlist *rlst, *clst;

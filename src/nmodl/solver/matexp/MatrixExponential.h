...
void matrix_exp_pade9(const MatA& A, MatU& U, MatV& V) {
    typedef typename MatA::PlainObject MatrixType;
    typedef typename NumTraits<typename traits<MatrixType>::Scalar>::Real RealScalar;
    const RealScalar b[] = {17643225600.L,
                            8821612800.L,
                            2075673600.L,
                            302702400.L,
                            30270240.L,
...
template <typename MatrixType,
          typename RealScalar = typename NumTraits<typename traits<MatrixType>::Scalar>::Real>
struct matrix_exp_computeUV {
     ...
     * denotes the matrix \c arg. The degree of the Pad&eacute; approximant and the value of
     * squarings are chosen such that the approximation error is no more than the round-off error.
     */
...
void matrix_exp_compute(const ArgType& arg, ResultType& result, true_type)  // natively supported
                                                                            // scalar type
{
    typedef typename ArgType::PlainObject MatrixType;
    MatrixType U, V;
    int squarings;
    matrix_exp_computeUV<MatrixType>::run(arg, U, V, squarings);  // Pade approximant is (U+V) /
                                                                  // (-U+V)
...
struct MatrixExponentialReturnValue: public ReturnByValue<MatrixExponentialReturnValue<Derived>> {
  public:
    ...
    inline void evalTo(ResultType& result) const {
        ...
        internal::matrix_exp_compute(tmp,
                                     result,

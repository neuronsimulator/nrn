...
void solv_diffeq(qsol, fun, method, numeqn, listnum, steadystate, btype) Item* qsol;
Symbol *fun, *method;
    ...
    if (method->subtype & DERF) {
        ...
        if (deriv_imp_list) { /* make sure deriv block translation matches method */
            ...
            if ((strcmp(method->name, Derivimplicit) == 0) ^ (found == 1)) {
                diag(
                    "To use the derivimplicit method the SOLVE statement must\
...
static Symbol* init_forderiv(prime) Symbol* prime;
{
    char name[256];
    double d1, d2;
...
static char* name_forderiv(i) int i;
{
...
void add_deriv_imp_list(name) char* name;
{
    if (!deriv_imp_list) {
...
void massagederiv(q1, q2, q3, q4, sensused) Item *q1, *q2, *q3, *q4;
int sensused;
{
    ...
    vectorize_substitute(q,
                         "(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int "
...
is not allowed on the left hand side.");
            ...
            if (s->subtype & ARRAY) {
                ...
                Sprintf(buf, "_slist%d[%d] = &(%s) - _p;", numlist, count, state->name);
                Lappendstr(initlist, buf);
                Sprintf(
    ...
    Lappendstr(procfunc, buf);
    {
        ...
        vectorize_substitute(
            qq->next,
            "(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0;");
...
void matchmassage(nderiv) int nderiv;
{
...
#if CVODE
static int cvode_linear_diffeq(ds, s, qbegin, qend) Symbol *ds, *s;
Item *qbegin, *qend;
{
    char* c;
    List* tlst;
...
static void cvode_diffeq(ds, qbegin, qend) Symbol* ds;
Item *qbegin, *qend;
{

...
void solv_partial(qsol, fun) Item* qsol;
Symbol* fun;
{
    int i, ident;
    Item* q;
    ...
    ITERATE(q, parinfo) {
        ...
        Sprintf(buf,
                "if (error=crank(%d, %s, %s, %s, delta_%s, %s, %s, _pbound%d, &_parwork%d))return "
                "error;\n",
                pv[2]->araydim,
                pv[2]->name,
                pv[1]->name,
    ...
    *q2, *q3, *q6;
{
    ...
    if (!pv[0]) {
        diag("within the PARTIAL block must occur at least one equation with the syntax ---\n",
             "~ V' = F*DEL2(V) + G\n");
    }
        ...
        for (i = 0; i < 2; i++) {
            if (bnd[i]) {         /* Neumann */
                if (bnd[i + 2]) { /* and also dirichlet */
                    diag("Neumann and Dirichlet conditions", " specified on same side");
                }
    ...
    ITERATE(qb, bndinfo) {
        ...
        qb = qb->next;
        if (qb->itemtype != -1) {
            diag("Boundary condition: No partial equation for ", s->name);
        }
    }
    replacstr(q1, "int");
    Insertstr(q3, "()\n");
...
void partial_bndry(type, qvar, qfirstlast, qexpr, qlast) int type;

...
void nrn_wrote_conc(sym, pe, it) Symbol* sym;
double* pe;
int it;
{
    if (it & 04) {
...
*/
void nrn_check_conc_write(p_ok, pion, i) Prop *p_ok, *pion;
int i;
{
...
void nrn_promote(p, conc, rev) Prop* p;
int conc, rev;
{
    int oldconc, oldrev;
    int* it = &p->dparam[0].i;
...
static void ion_alloc(p) Prop* p;
{
    double* pd[1];
    int i = 0;

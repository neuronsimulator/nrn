...
struct NrnThread {
    ...
    // Pointer to cache data generated by neuron::cache::acquire_valid and
    // friends. This will be non-null only when a valid cache exists.
    neuron::cache::Model *cache{};
    neuron::cache::Mechanism& mech_cache(int mech_type) {
        if (nrn_is_artificial_[mech_type]) {
            assert(cache);
            return cache->art_cell[mech_type];
        } else {
            return thread_cache().mech[mech_type];
        }
    }
    neuron::cache::Thread& thread_cache() {
        assert(cache);

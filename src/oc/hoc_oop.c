...
Object* hoc_newobj1(Symbol* sym, int narg) {
    ...
    if (sym->subtype & (CPLUSOBJECT | JAVAOBJECT)) {
        ...
        ob->u.dataspace = obd = (Objectdata*) ecalloc(ob->template->dataspace_size,
                                                      sizeof(Objectdata));
                ...
                for (i = 0; i < total; i++) {
                    ...
                    if (*obp) {
                        ...
                        if ((*obp)->template == template) {
                            ...
                            hoc_free_allobjects(template,
                                                (*obp)->template->symtable,
                                                (*obp)->u.dataspace);
                        }
#else
...
static void free_objectdata(Objectdata* od, Template* template) {
    ...
    psav = hoc_objectdata;
    hoc_objectdata = od;
    if (template->symtable)
        for (s = template->symtable->first; s; s = s->next) {
            if (s->public != 2) {
                switch (s->type) {
                case VAR:
            ...
            if (s->type == OBJECTVAR && s->public != 2) {
                ...
                for (i = 0; i < total; i++) {
                    ...
                    if (*obp && !(*obp)->recurse && s->subtype != CPLUSOBJECT &&
                        ...
                        /* descend to look for more */
                        (*obp)->recurse = 1;
                        hoc_list_allobjref((*obp)->template->symtable,
                                           (*obp)->u.dataspace,
                                           depth + 1);
                        (*obp)->recurse = 0;
                    }
...
void check_obj_type(Object* obj, const char* typename) {
    ...
    if (!obj || strcmp(obj->template->sym->name, typename) != 0) {
        if (obj) {
            sprintf(buf, "object type is %s instead of", obj->template->sym->name);
        } else {

...
/* Summary of menu functions :
    insert_menu(int r, int c, sym *sp ) - Stores row and col coordinates, and the symbol pointer
specified in fcursor() as an item in a structure list. Successive items are appended at the end of
the list. This function returns the pointer for the beginning of the list. display_menu(Menuitem
*menu) - Displays the whole menu as specified by successive calls to fcursor. Recovers information
stored for each menu item and prints the corresponding item variable name and value at its specified
position. destroy_menu(Menuitem *menu) - Frees the space allocated for the whole menu list.
    navigate_menu(Menuitem *menu) - Allows user to move around the displayed menu by making use of
the arrow keys. erase_item(Menuitem *pnow) - Erases second line contents in menu
*/
...
void hoc_fmenu(void) {
    ...
    case -1:
        ...
        maxmenus = imenu;
        for (i = 0; i < maxmenus; i++) {
            menusfirst[i] = menuslast[i] = menuscurrent[i] = (Menuitem*) 0;
        }
        break;
    case 0:
        chk(imenu);
        ...
        if (key == 0) { /* arrow key pressed */
            ...
            default:
                ...
                break;
                   ...
                   ((isdigit(key) || key == '-' || key == '+' || key == 015 || key == '.'))) {
            prs(0, row, col, "");

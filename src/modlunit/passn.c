...
int next_intoken(pitem) Item** pitem;
{
    if (parse_pass == 1) {
        return yylex();
    }
...
Item* next_parstok(intok) Item* intok;
{
    if (!intok) {
        return ITEM0;
    }
...
Item* prev_parstok(intok) Item* intok;
{
    if (!intok) {
        return ITEM0;
    }

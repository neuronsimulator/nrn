...
void ucopypop(up) struct unit* up;
{
    int i;
    for (i = 0; i < NDIM; i++) {
        up->dim[i] = usp->dim[i];
...
void ucopypush(up) struct unit* up;
{
    int i;
    Unit_push("");
    for (i = 0; i < NDIM; i++) {
...
void Unit_push(string) char* string;
{
    IFUNITS
...
void unit_push_num(d) double d;
{
    Unit_push("");
    usp->factor = d;
}

void unitcheck(s) char* s;
{
    Unit_push(s);
    unit_pop();
}
...
void Unit_exponent(val) int val;
{
...
static void units(up) struct unit* up;
{ printf("\t%s\n", Unit_str(up)); }

static char* ucp;
char* Unit_str(up) struct unit* up;
{
...
static int convr(up) struct unit* up;
{
    register struct unit* p;
    register int c;
    register char* cp;
...
static int lookup(name, up, den, c) char* name;
struct unit* up;
int den, c;
{
    register struct unit* p;
...
static int equal(s1, s2) char *s1, *s2;
{
    register char *c1, *c2;
...
static struct table* hash(name) char* name;
{

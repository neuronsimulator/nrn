...
void unit_push(q) Item* q;
{ Unit_push(decode_units(SYM(q))); }

char* decode_units(sym) Symbol* sym;
{
    if (sym->u.str) {
        return sym->u.str;
    }
...
static Item* qexpr[3];
void unit_cmp(q1, q2, q3) Item *q1, *q2, *q3;
{
    qexpr[0] = q1;
    qexpr[1] = q2;
    qexpr[2] = q3;
...
void unit_logic(type, q1, q2, q3) int type;
Item *q1, *q2, *q3;
{
...
void unit_push_args(q1) Item* q1;
{
    List* larg;
    Item* q;
    Symbol* s;
...
void unit_chk_arg(q1, q2) Item *q1, *q2;
{
    if (argnumstk[pargnum] > 0) {
        argnumstk[pargnum]--;
...
void func_unit(q1, q2) Item *q1, *q2;
{
    Symbol *s, *checklocal();

...
void kinunits(type, pass) Item* type;
int pass;
{
...
static void set_flux_units(up) struct unit* up;
{
    Symbol* s;
...
static void react_unit_err(s, up) char* s;
struct unit* up;
{
...
void unit_compartlist(q) Item* q;
{
    char* ustr;
...
void unit_ldifuslist(q, flag) Item* q;
int flag;
{
    char* ustr;
    unitonflag = flag;
    ...
    if (!unit_cmp_exact()) {
        diag(ustr,
             ": With LONGDITUDINAL_DIFFUSION the compartment \

...
#endif
extern char *stralloc(),                   /* copies string to new space */
    *inputline(),                          /* used only by parser to get title line */
    *inputtopar(),                         /* used only by parser to get units */
    *decode_units(), *unit_str(), *Gets(); /* used only in io.c to get string from fin. */
...
extern Item *putintoken(),       /* construct symbol and store input tokens */
    *insertstr(),                /* before a known Item */
    *insertsym(), *linsertstr(), /* prepend to list */
    *lappendstr(),               /* append to list */
    *linsertsym(), *lappendsym(), *lappenditem(), *listtype(), *next_parstok(), *prev_parstok(),
    *car(), *next(), *prev();

#include "modlunit.h" /* void functions */

extern Symbol *install(),  /* Install token in symbol table */

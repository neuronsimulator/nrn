...
char* Fgets(buf, size, f) char* buf;
int size;
FILE* f;
{
    char* p = buf;
...
Symbol* _SYM(q, file, line) Item* q;
char* file;
int line;
{
...
void internal_error(q, file, line) Item* q;
char* file;
int line;
{
...
static int getprefix(prefix, s) char *prefix, *s;
{
    char* cp;
    strcpy(prefix, s);
...
static FILE* include_open(fname, err) char* fname;
int err;
{
    FILE* f = (FILE*) 0;
    FileStackItem* fsi;

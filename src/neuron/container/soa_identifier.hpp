  ...
  private:
    ...
    void destroy() {
        ...
        try {
            ...
            Fprintf(stderr, fmt::format("neuron::container::owning_identifier<{}> destructor could not delete from the underlying storage: {} [{}]. This is not recoverable, aborting.",
                      cxx_demangle(typeid(Storage).name()),
                      e.what(),
                      cxx_demangle(typeid(e).name())).c_str());
            terminate = true;
        }
        if (terminate) {
            std::terminate();

            ...
            if (_rxd_induced_currents_grid[i] == id) {
                ...
                for (j = 0; j < grid->num_all_currents; j++) {
                    if (ptrs[i]->u.px_ == c[j].source) {
                        _rxd_induced_currents_scale[i] =
                            c[j].scale_factor / (grid->VARIABLE_ECS_VOLUME == VOLUME_FRACTION
                                                     ? grid->alpha[c[j].destination]
                                                     : grid->alpha[0]);
...
extern "C" void setup_solver(double* my_states,
                             int my_num_states,
                             long* zvi,
                             int num_zvi,
                             PyHocObject* h_t_ref,
                             PyHocObject* h_dt_ref) {
    states = my_states;
...
extern "C" void set_reaction_indices(int num_locations,
                                     int* regions,
                                     int* num_species,
                                     int* species_index,
                                     int* indices,
                                     ...
                                     int num_ecs_species,
                                     int* ecs_grid_ids,
                                     int* ecs_species_counts,
                                     int* ecs_species_grid_ids,
                                     int* ecs_indices) {
    int i, j, k, r, idx;
    Grid_node* grid;

...
ECS_Grid_node* ECS_make_Grid(PyHocObject* my_states,
                             int my_num_states_x,
                             int my_num_states_y,
                             int my_num_states_z,
                             double my_dc_x,
    ...
    new_Grid->states_x = (double*) malloc(sizeof(double) * my_num_states_x * my_num_states_y *
                                          my_num_states_z);
    new_Grid->states_y = (double*) malloc(sizeof(double) * my_num_states_x * my_num_states_y *
                                          my_num_states_z);
    new_Grid->states_cur = (double*) malloc(sizeof(double) * my_num_states_x * my_num_states_y *
                                            my_num_states_z);
    ...
    for (k = 0; k < NUM_THREADS; k++) {
        new_Grid->ecs_tasks[k].scratchpad = (double*) malloc(
...
int ECS_insert(int grid_list_index,
    ...
    Grid_node* new_Grid = ECS_make_Grid(my_states,
                                        my_num_states_x,
                                        my_num_states_y,
                                        my_num_states_z,
                                        my_dc_x,
...
Grid_node* ICS_make_Grid(PyHocObject* my_states,
                         long num_nodes,
                         long* neighbors,
                         long* ordered_x_nodes,
                         long* ordered_y_nodes,
                         long* ordered_z_nodes,
                         long* x_line_defs,
                         long x_lines_length,
                         long* y_line_defs,
                         long y_lines_length,
    ...
    for (int k = 0; k < NUM_THREADS; k++) {
        new_Grid->ics_tasks[k].RHS = (double*) malloc(sizeof(double) *
                                                      (new_Grid->_line_length_max));
        new_Grid->ics_tasks[k].scratchpad = (double*) malloc(sizeof(double) *
                                                             (new_Grid->_line_length_max - 1));
        new_Grid->ics_tasks[k].g = new_Grid;
        new_Grid->ics_tasks[k].u_diag = (double*) malloc(
            sizeof(double) * new_Grid->_line_length_max - 1);
        new_Grid->ics_tasks[k].diag = (double*) malloc(sizeof(double) * new_Grid->_line_length_max);
        new_Grid->ics_tasks[k].l_diag = (double*) malloc(
            sizeof(double) * new_Grid->_line_length_max - 1);
    }
    ...
    new_Grid->ics_adi_dir_x->ordered_start_stop_indices = (long*) malloc(sizeof(long) *
                                                                         NUM_THREADS * 2);
    new_Grid->ics_adi_dir_x->line_start_stop_indices = (long*) malloc(sizeof(long) * NUM_THREADS *
                                                                      2);
    new_Grid->ics_adi_dir_x->ordered_nodes = (long*) malloc(sizeof(long) * new_Grid->_num_nodes);
    new_Grid->ics_adi_dir_x->ordered_line_defs = (long*) malloc(sizeof(long) *
                                                                new_Grid->_x_lines_length);
    ...
    new_Grid->ics_adi_dir_y->ordered_start_stop_indices = (long*) malloc(sizeof(long) *
                                                                         NUM_THREADS * 2);
    new_Grid->ics_adi_dir_y->line_start_stop_indices = (long*) malloc(sizeof(long) * NUM_THREADS *
                                                                      2);
    new_Grid->ics_adi_dir_y->ordered_nodes = (long*) malloc(sizeof(long) * new_Grid->_num_nodes);
    new_Grid->ics_adi_dir_y->ordered_line_defs = (long*) malloc(sizeof(long) *
                                                                new_Grid->_y_lines_length);
    ...
    new_Grid->ics_adi_dir_z->ordered_start_stop_indices = (long*) malloc(sizeof(long) *
                                                                         NUM_THREADS * 2);
    new_Grid->ics_adi_dir_z->line_start_stop_indices = (long*) malloc(sizeof(long) * NUM_THREADS *
                                                                      2);
    new_Grid->ics_adi_dir_z->ordered_nodes = (long*) malloc(sizeof(long) * new_Grid->_num_nodes);
    new_Grid->ics_adi_dir_z->ordered_line_defs = (long*) malloc(sizeof(long) *
                                                                new_Grid->_z_lines_length);
...
int ICS_insert(int grid_list_index,
    ...
    Grid_node* new_Grid = ICS_make_Grid(my_states,
                                        num_nodes,
                                        neighbors,
                                        ordered_x_nodes,
                                        ordered_y_nodes,
...
extern "C" void ics_set_grid_concentrations(int grid_list_index,
    ...
    int total_surface_nodes = nodes_per_seg_start_indices[n];  // nodes_per_seg_lengths has length n
                                                               // + 1 since it has a 0 as the first
                                                               // start index

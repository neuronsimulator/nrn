...
#define Fxx(x1, x2)                                          \
    (ALPHA(x1, y, z) * ALPHA(x2, y, z) * g->dc_x *           \
     (g->states[IDX(x1, y, z)] - g->states[IDX(x2, y, z)]) / \
     ((ALPHA(x1, y, z) + ALPHA(x2, y, z)) * LAMBDA(x1, y, z)))
#define Fxy(y1, y1d, y2)                                     \
    (ALPHA(x, y1, z) * ALPHA(x, y2, z) * g->dc_y *           \
     (g->states[IDX(x, y1, z)] - g->states[IDX(x, y2, z)]) / \
     ((ALPHA(x, y1, z) + ALPHA(x, y2, z)) * LAMBDA(x, y1d, z)))
#define Fxz(z1, z1d, z2)                                     \
    (ALPHA(x, y, z1) * ALPHA(x, y, z2) * g->dc_z *           \
     (g->states[IDX(x, y, z1)] - g->states[IDX(x, y, z2)]) / \
     ((ALPHA(x, y, z1) + ALPHA(x, y, z2)) * LAMBDA(x, y, z1d)))
#define Fyy(y1, y2)                                          \
    (ALPHA(x, y1, z) * ALPHA(x, y2, z) * g->dc_y *           \
     (g->states[IDX(x, y1, z)] - g->states[IDX(x, y2, z)]) / \
     ((ALPHA(x, y1, z) + ALPHA(x, y2, z)) * LAMBDA(x, y1, z)))
#define Fzz(z1, z2)                                          \
    (ALPHA(x, y, z1) * ALPHA(x, y, z2) * g->dc_z *           \
     (g->states[IDX(x, y, z1)] - g->states[IDX(x, y, z2)]) / \
     ((ALPHA(x, y, z1) + ALPHA(x, y, z2)) * LAMBDA(x, y, z1)))
    ...
    if (g->size_x == 1) {
        if (g->bc->type == DIRICHLET) {
            ...
            RHS[0] = g->states[IDX(0, y, z)] +
                     (dt / ALPHA(0, y, z)) * ((Fxy(yp, ypd, y) - Fxy(y, ymd, ym)) / div_y +
...
static void ecs_dg_adi_tort_x(ECS_Grid_node* g,
    ...
    if (g->size_x == 1) {
        if (g->bc->type == DIRICHLET) {
            ...
            RHS[0] = g->states[IDX(0, y, z)] +
                     dt * (g->states_cur[IDX(0, y, z)] +
                           (DcY(0, ypd, z) * g->states[IDX(0, yp, z)] -
                            (DcY(0, ypd, z) + DcY(0, ymd, z)) * g->states[IDX(0, y, z)] +
                            DcY(0, ymd, z) * g->states[IDX(0, ym, z)]) /
                               (div_y * SQ(g->dy)) +
                           (DcZ(0, y, zpd) * g->states[IDX(0, y, zp)] -
                            (DcZ(0, y, zpd) + DcZ(0, y, zmd)) * g->states[IDX(0, y, z)] +
                            DcZ(0, y, zmd) * g->states[IDX(0, y, zm)]) /
                               (div_z * SQ(g->dz)));
        }
        return;
    }
    ...
    if (g->bc->type == NEUMANN) {
        ...
        RHS[0] = g->states[IDX(0, y, z)] +
                 dt * ((DcX(1, y, z) * g->states[IDX(1, y, z)] -
                        DcX(1, y, z) * g->states[IDX(0, y, z)]) /
                           (2. * SQ(g->dx)) +
                       (DcY(0, ypd, z) * g->states[IDX(0, yp, z)] -
                        (DcY(0, ypd, z) + DcY(0, ymd, z)) * g->states[IDX(0, y, z)] +
                        DcY(0, ymd, z) * g->states[IDX(0, ym, z)]) /
        ...
        x = g->size_x - 1;
        RHS[x] = g->states[IDX(x, y, z)] +
                 dt * ((DcX(x, y, z) * g->states[IDX(x - 1, y, z)] -
                        DcX(x, y, z) * g->states[IDX(x, y, z)]) /
                           (2. * SQ(g->dx)) +
                       (DcY(x, ypd, z) * g->states[IDX(x, yp, z)] -
                        (DcY(x, ypd, z) + DcY(x, ymd, z)) * g->states[IDX(x, y, z)] +
                        DcY(x, ymd, z) * g->states[IDX(x, ym, z)]) /
                           (div_y * SQ(g->dy)) +
                       (DcZ(x, y, zpd) * g->states[IDX(x, y, zp)] -
                        (DcZ(x, y, zpd) + DcZ(x, y, zmd)) * g->states[IDX(x, y, z)] +
                        DcZ(x, y, zmd) * g->states[IDX(x, y, zm)]) /
                           (div_z * SQ(g->dz))) +
                 dt * g->states_cur[IDX(x, y, z)];
    ...
    for (x = 1; x < g->size_x - 1; x++) {
        ...
        RHS[x] = g->states[IDX(x, y, z)] +
                 dt * ((DcX(x + 1, y, z) * g->states[IDX(x + 1, y, z)] -
                        (DcX(x + 1, y, z) + DcX(x, y, z)) * g->states[IDX(x, y, z)] +
                        DcX(x, y, z) * g->states[IDX(x - 1, y, z)]) /
                           (2. * SQ(g->dx)) +
...
void _rhs_variable_step_helper_tort(Grid_node* g, double const* const states, double* ydot) {
    ...
    if (g->bc->type == NEUMANN) {
        ...
        for (i = 0, index = 0, next_i = num_states_y * num_states_z; i < num_states_x; i++) {
            for (j = 0, prev_j = index - num_states_z, next_j = index + num_states_z;
                 j < num_states_y;
                 j++) {
                for (k = 0, prev_k = index - 1, next_k = index + 1; k < num_states_z;
        ...
        for (i = 0,
            ...
            for (j = 0, prev_j = index + num_states_z, next_j = index + num_states_z;
                ...
                for (k = 0, prev_k = index + 1, next_k = index + 1; k < num_states_z;
                    ...
                    /*x-direction*/
                    ydot[index] += rate_x *
                                   (FLUX(next_i, index) / LAMBDA(xpd, j, k) +
                                    FLUX(prev_i, index) / LAMBDA(xmd, j, k)) /
                                   (VOLFRAC(index) * div_x);

                    /*y-direction*/
                    ydot[index] += rate_y *
                                   (FLUX(next_j, index) / LAMBDA(i, ypd, k) +
                                    FLUX(prev_j, index) / LAMBDA(i, ymd, k)) /
                                   (VOLFRAC(index) * div_y);

                    /*z-direction*/
                    ydot[index] += rate_z *
                                   (FLUX(next_k, index) / LAMBDA(i, j, zpd) +
                                    FLUX(prev_k, index) / LAMBDA(i, j, zmd)) /
                                   (VOLFRAC(index) * div_z);
            ...
            for (j = 0, prev_j = index - num_states_z, next_j = index + num_states_z;
                ...
                for (k = 0, prev_k = index - 1, next_k = index + 1; k < num_states_z;
                    ...
                    if (i == 0 || i == stop_i || j == 0 || j == stop_j || k == 0 || k == stop_k) {
                        ...
                        /*x-direction*/
                        ydot[index] += rate_x *
                                       (FLUX(next_i, index) / LAMBDA(i + 1, j, k) +
                                        FLUX(prev_i, index) / LAMBDA(i, j, k)) /
                                       (VOLFRAC(index));

                        /*y-direction*/
                        ydot[index] += rate_y *
                                       (FLUX(next_j, index) / LAMBDA(i, j + 1, k) +
                                        FLUX(prev_j, index) / LAMBDA(i, j, k)) /
                                       (VOLFRAC(index));

                        /*z-direction*/
                        ydot[index] += rate_z *
                                       (FLUX(next_k, index) / LAMBDA(i, j, k + 1) +
                                        FLUX(prev_k, index) / LAMBDA(i, j, k)) /
                                       (VOLFRAC(index));
                    }
                }
                prev_j = index - num_states_z;

        ...
        if (i == species_ids[0]) {
            if (mc3d_region_size > 0) {
                ...
                memcpy(r->mc3d_indices_offsets,
                       mc3d_start_indices,
                       sizeof(double) * (num_species + num_params));
...
#if NRNMPI
    if (nrnmpi_use) {
        nrnmpi_dbl_allgatherv_inplace(grid->all_currents,
                                      grid->proc_num_currents,
                                      grid->proc_offsets);
        for (i = 0; i < n; i++)
        ...
        for (i = 0; i < _memb_curr_total; i++) {
            if (_rxd_induced_currents_grid[i] == grid_id) {
                if (_rxd_induced_currents_ecs_idx[i] != SPECIES_ABSENT)
                    output[_rxd_induced_currents_ecs_idx[i]] -=
                        dt * (_rxd_induced_currents_ecs[i] * _rxd_induced_currents_scale[i]);
            }
        }
    }
}
...
void _rhs_variable_step_ecs(const double t,
                            const double* states,
                            double* ydot,
                            const int _cvode_offset) {
    Grid_node* grid;
    ssize_t i;
    int grid_size;
    double dt = *dt_ptr;
        ...
        for (i = 0,
            ...
            for (j = 0, prev_j = index + num_states_z, next_j = index + num_states_z;
                ...
                for (k = 0, prev_k = index + 1, next_k = index + 1; k < num_states_z;
                    ...
                    ydot[index] += rate_x *
                                   (states[prev_i] - 2.0 * states[index] + states[next_i]) / div_x;

                    ydot[index] += rate_y *
                                   (states[prev_j] - 2.0 * states[index] + states[next_j]) / div_y;

                    ydot[index] += rate_z *
                                   (states[prev_k] - 2.0 * states[index] + states[next_k]) / div_z;
...
static void ecs_dg_adi_x(ECS_Grid_node* g,
    ...
    if (g->bc->type == NEUMANN) {
        ...
        RHS[0] = g->states[IDX(0, y, z)] +
                 dt * ((g->dc_x / SQ(g->dx)) *
                           (g->states[IDX(1, y, z)] - 2. * g->states[IDX(0, y, z)] +
                            g->states[IDX(1, y, z)]) /
                           4.0 +
                       (g->dc_y / SQ(g->dy)) *
                           (g->states[IDX(0, yp, z)] - 2. * g->states[IDX(0, y, z)] +
                            g->states[IDX(0, ym, z)]) /
                           div_y +
                       (g->dc_z / SQ(g->dz)) *
                           (g->states[IDX(0, y, zp)] - 2. * g->states[IDX(0, y, z)] +
                            g->states[IDX(0, y, zm)]) /
                           div_z) +
        ...
        RHS[x] = g->states[IDX(x, y, z)] +
                 dt * ((g->dc_x / SQ(g->dx)) *
                           (g->states[IDX(x - 1, y, z)] - 2. * g->states[IDX(x, y, z)] +
                            g->states[IDX(x - 1, y, z)]) /
                           4.0 +
                       (g->dc_y / SQ(g->dy)) *
                           (g->states[IDX(x, yp, z)] - 2. * g->states[IDX(x, y, z)] +
                            g->states[IDX(x, ym, z)]) /
                           div_y +
                       (g->dc_z / SQ(g->dz)) *
                           (g->states[IDX(x, y, zp)] - 2. * g->states[IDX(x, y, z)] +
                            g->states[IDX(x, y, zm)]) /
                           div_z) +

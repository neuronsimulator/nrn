...
static int NPySecObj_init(NPySecObj* self, PyObject* args, PyObject* kwds) {
    ...
    if (self != NULL && !self->sec_) {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kwds, "|Os", (char**) kwlist, &self->cell_, &name)) {
            return -1;
        }
...
static PyMethodDef NPyRangeVar_methods[] =
    {{"name", (PyCFunction) NPyRangeVar_name, METH_NOARGS, "Range variable array name"}, {NULL}};
...
myPyMODINIT_FUNC nrnpy_nrn(void) {
    ...
    if (PyType_Ready(pallsegiter_type) < 0)
        goto fail;
...
#else
    m = Py_InitModule3("_neuron_section", nrnpy_methods, "NEURON interaction with Python");
#endif

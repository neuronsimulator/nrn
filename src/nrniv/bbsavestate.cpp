...
extern void nrnmpi_dbl_alltoallv(double *, int *, int *, double *, int *,
                                 int *);
...
static void nrnmpi_int_alltoallv(int *s, int *scnt, int *sdispl, int *r,
                                 int *rcnt, int *rdispl) {
  for (int i = 0; i < scnt[0]; ++i) {
    r[i] = s[i];
  }
}
static void nrnmpi_dbl_alltoallv(double *s, int *scnt, int *sdispl, double *r,
                                 int *rcnt, int *rdispl) {
  for (int i = 0; i < scnt[0]; ++i) {
    r[i] = s[i];
  }
...
extern "C" void *bbss_buffer_counts(int *len, int **gids, int **sizes,
                                    int *global_size);
...
extern "C" void bbss_restore(void *bbss, int gid, int npiece, char *buffer,
                             int sz);
...
BBSS_TxtFileOut::~BBSS_TxtFileOut() { fclose(f); }
...
BBSS_TxtFileIn::~BBSS_TxtFileIn() { fclose(f); }
...
extern "C" void bbss_restore(void *bbss, int gid, int ngroup, char *buffer,
                             int sz) {
  usebin_ = 1;
  BBSaveState *ss = (BBSaveState *)bbss;
  BBSS_IO *io = new BBSS_BufferIn(buffer, sz);
...
static Member_func members[] = {
    // text test
    "save", save, "restore", restore, "save_test", save_test, "restore_test",
    restore_test,
    // binary test
    "save_test_bin", save_test_bin, "restore_test_bin", restore_test_bin,
    // binary save/restore interface to interpreter
    "save_request", save_request, "save_gid", save_gid, "restore_gid",
    restore_gid,
    // indicate which point processes are to be ignored
    "ignore", ppignore,
    // allow Vector.play to work
    "vector_play_init", vector_play_init, 0, 0};
...
static void ssi_def() {
  ...
  for (int im = 0; im < n_memb_func; ++im) {
    ...
    if (pnt_receive[im]) {
      ...
      int type = STATE;
      for (Symbol *sym = np->first_var(); np->more_var();
           sym = np->next_var()) {
...
SEWrap::SEWrap(const TQItem *tq, DEList *dl) {
  ...
  if (se->weight_) {
    ncindex = 0;
    for (; dl && dl->de && dl->de->type() == NetConType;
         dl = dl->next, ++ncindex) {
...
static void pycell_name2sec_maps_clear() { pycell_name2sec_maps.clear(); }
...
static void pycell_name2sec_maps_fill() {
  ...
  ForAllSections(sec) // macro has the {
      if (sec->prop &&
          sec->prop->dparam[PROP_PY_INDEX]._pvoid) { // PythonSection
    ...
    hoc_execerr_ext("Python Section, %s, not associated with Python Cell.",
                    secname(sec));
  }
}
}
  ...
  if (!is_point_process(c)) {       // must be cell object
    if (f->type() != BBSS_IO::IN) { // writing, counting
      ...
      if (qsec) { // Write HOC Cell
        for (first = qsec;
             first->itemtype && hocSEC(first)->prop->dparam[6].obj == c;
             first = first->prev) {
          sec = hocSEC(first);
          if (sec->prop) {
            ++cnt;
...
void BBSaveState::netrecv_pp(Point_process *pp) {
  ...
  // NetCon
  for (cnt = 0, dl1 = dl; dl1 && dl1->de->type() == NetConType;
       dl1 = dl1->next) {
    ++cnt;
  }
  f->i(cnt, 1);
  ...
  for (cnt = 0, dl1 = dl; dl1 && dl1->de->type() == SelfEventType;
       dl1 = dl1->next) {
    ++cnt;
  }
  f->i(cnt);
  ...
  if (f->type() != BBSS_IO::IN) {
    ...
    for (int i = 0; i < cnt; ++i) {
      ...
      assert(tqi && tqi->data_ &&
             ((DiscreteEvent *)tqi->data_)->type() == SelfEventType);
...
static void scatteritems() {
  ...
  double *tssrc =
      0; // all the doubles (and undelivered NetCon counts) in every DblList
  if (src2send_cnt) {
    int ndsrctotal = 0;
...
static void spikes_on_correct_host(int cnt, int *g, int *dcnts, int tscnt,
                                   double *ts, Int2DblList *m) {
      ...
      if (dl2iter != presyn_queue->end()) {
        ...
        if (dl->size() == dl2->size()) {
          for (int i = 0; i < dl->size(); i += 2) {
            if ((fabs((*dl)[i] - (*dl2)[i]) > 1e-12) ||
                (*dl)[i + 1] != (*dl2)[i + 1]) {
              printf("error: gid=%d expect t=%g %d but queue contains t=%g %d  "
                     "tdiff=%g\n",
                     gid, (*dl)[i], int((*dl)[i + 1]), (*dl2)[i],
                     int((*dl2)[i + 1]), (*dl)[i] - (*dl2)[i]);
            }
          }
        } else {
          printf(

...
declarePtrList(TQItemList, TQItem)
    implementPtrList(TQItemList, TQItem) static TQItemList* tq_presyn_fanout;
...
declareNrnHash(PointProcessMap, Point_process*, int)
    implementNrnHash(PointProcessMap, Point_process*, int) static PointProcessMap* pp_ignore_map;
...
declareNrnHash(PP2DE, Point_process*, DEList*)
    implementNrnHash(PP2DE, Point_process*, DEList*) static PP2DE* pp2de;
// NetCon.events
declareList(DblList, double) implementList(DblList, double)
    declareNrnHash(NetCon2DblList, NetCon*, DblList*)
        implementNrnHash(NetCon2DblList, NetCon*, DblList*) static NetCon2DblList* nc2dblist;
...
declarePtrList(SEWrapList, SEWrap)
    implementPtrList(SEWrapList, SEWrap) static SEWrapList* sewrap_list;

declareNrnHash(Int2Int, int, int)
    implementNrnHash(Int2Int, int, int) static Int2Int* base2spgid;  // base gids are the host
                                                                     // independent key for a cell
                                                                     // which was multisplit

declareNrnHash(Int2DblList, int, DblList*)
    implementNrnHash(Int2DblList, int, DblList*) static Int2DblList* src2send;  // gid to presyn
                                                                                // send time map
...
void BBSaveState::possible_presyn(int gid) {
    ...
    if (f->type() != BBSS_IO::IN) {  // save or count
       ...
       // record vector size.
                ...
                // skip -> file has undelivered spikes, but this is not the cell that deals with
                // them
                f->i(cnt);
                for (int j = 0; j < cnt; j += 2) {
                    f->d(1, ts);
        ...
        if (presyn_queue->find(gid, dl2)) {
            if (dl->count() == dl2->count()) {
                for (int i = 0; i < dl->count(); i += 2) {
                    if ((fabs(dl->item(i) - dl2->item(i)) > 1e-12) ||
                        dl->item(i + 1) != dl2->item(i + 1)) {
                        printf(
                            "error: gid=%d expect t=%g %d but queue contains t=%g %d  tdiff=%g\n",
                            gid,
                            dl->item(i),
                            int(dl->item(i + 1)),
                            dl2->item(i),
                            int(dl2->item(i + 1)),
                            dl->item(i) - dl2->item(i));
                    }
                }
            } else {

...
declareNrnHash(Gid2PreSyn, int, PreSyn*) implementNrnHash(Gid2PreSyn, int, PreSyn*)
...
// printf("nrn_spike_exchange_init\n");
    ...
    if (nrnmpi_use) {
        active_ = 1;
    }  // in case set_mindelay has not yet been called
#endif
    if (!nrn_need_npe()) {
        return;
    }
...
void BBS::cell() {
    ...
    ps = nc->src_;
// printf("%d cell %d %s\n", nrnmpi_myid, gid, hoc_object_name(ps->ssrc_ ? nrn_sec2cell(ps->ssrc_) :
// ps->osrc_));
#if ALTHASH

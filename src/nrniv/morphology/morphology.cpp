...
namespace neuron::morphology {
    ...
    std::string type2name(int type_id) {
        ...
        if (it != type2name_dict.end()) {
            ...
            return (type_id < 0) ? "minus_" + std::to_string(-type_id) : "dend_" + std::to_string(type_id);
        }
    }

    std::string mksubset(int type_id, int freq, const std::string& type_name) {
        static const std::unordered_map<int, std::string> mksubset_dict = {
            {1, "somatic"},
            {2, "axonal"},
            {3, "basal"},
            {4, "apical"}
        ...
        std::string tstr = (it != mksubset_dict.end()) ? it->second : ((type_id < 0) ? "minus_" + std::to_string(-type_id) + "set" : "dendritic_" + std::to_string(type_id));

...
void nrn_connect_sections(Section* child_sec, double child_x, Section* parent_sec, double parent_x) {
...
void nrn_set_section_length(Section* sec, double length) {
    // TODO: call can_change_morph(sec) to check pt3dconst_; how should we handle that?
...
int nrn_get_symbol_type(Symbol* sym) {
    // TODO: these types are in parse.hpp and are not the same between versions, so we really should wrap
    return sym->type;
}

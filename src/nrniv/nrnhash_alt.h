    ...
    class NrnHashIterator(NrnHash) {                      \
      ...
      private:                                            \
        ...
        NrnHashEntry(NrnHash) * *last_;                   \
                                                          ...
                                                          \
    inline Key& NrnHashIterator(NrnHash)::cur_key() {     \
        return cur_->key_;                                \
    }                                                     \
    inline Value& NrnHashIterator(NrnHash)::cur_value() { \
        return cur_->value_;                              \
    }                                                     \
    inline bool NrnHashIterator(NrnHash)::more() {        \
        return entry_ <= last_;                           \
    }
...
inline uint32_t nrn_key_to_hash(long k) {
    return nrn_uint32hash((uint32_t) k);
}
#if defined(__SIZEOF_POINTER__) && __SIZEOF_POINTER__ > __SIZEOF_LONG__
inline uint32_t nrn_key_to_hash(const void* k) {
    return nrn_uint32hash((uint32_t)((unsigned long long) k));
}
#else
inline uint32_t nrn_key_to_hash(const void* k) {
    return nrn_uint32hash((uint32_t)((unsigned long) k));
}
#endif
...
#define NrnHashIterate(Table, table, Value, value)                         \
    if (table)                                                             \
        for (NrnHashIterator(Table) i__(*table); i__.more(); i__.next()) { \
            {                                                              \

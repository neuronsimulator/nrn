...
static int nrnthread_dat1(int tid,
                          int& n_presyn,
                          int& n_netcon,
                          int*& output_gid,
                          int*& netcon_srcgid);
static int nrnthread_dat2_1(int tid,
                            int& ngid,
                            int& n_real_gid,
                            int& nnode,
                            int& ndiam,
                            ...
                            int& nvdata,
                            int& nweight);
static int nrnthread_dat2_2(int tid,
                            int*& v_parent_index,
                            double*& a,
                            double*& b,
                            double*& area,
                            double*& v,
                            double*& diamvec);
static int nrnthread_dat2_mech(int tid,
                               size_t i,
                               int dsz_inst,
                               int*& nodeindices,
                               double*& data,
                               int*& pdata);
static int nrnthread_dat2_3(int tid,
                            int nweight,
                            int*& output_vindex,
...
static int nrnthread_dat2_corepointer_mech(int tid,
                                           int type,
                                           int& icnt,
                                           int& dcnt,
                                           int*& iarray,
...
static int nrnthread_dat2_vecplay_inst(int tid,
                                       int i,
                                       int& vptype,
                                       int& mtype,
                                       int& ix,
...
declareNrnHash(PVoid2Int, void*, int)
    implementNrnHash(PVoid2Int, void*, int) PVoid2Int* artdata2index_;
...
int nrn_dblpntr2nrncore(double* pd, NrnThread& nt, int& type, int& index) {
    ...
    } else if (pd >= nt._nrn_fast_imem->_nrn_sav_rhs &&
...
static void write_globals(const char* fname) {
    ...
    for (void* sp = NULL; (sp = get_global_dbl_item(sp, name, size, val)) != NULL;) {
        if (size) {
            fprintf(f, "%s[%d]\n", name, size);
...
void write_nrnthread_task(const char* path, CellGroup* cgs) {
    ...
    if (nrnmpi_myid == 0) {
        ...
        if (!fp) {
            hoc_execerror("nrnbbcore_write write_nrnthread_task could not open for writing:",
                          filename.c_str());
        }

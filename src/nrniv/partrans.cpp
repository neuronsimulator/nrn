...
declarePtrList(DblPList, double) implementPtrList(DblPList, double) declarePtrList(NodePList, Node)
    implementPtrList(NodePList, Node)
#define PPList partrans_PPList
        declarePtrList(PPList, Point_process) implementPtrList(PPList, Point_process)
            declareList(IntList, int) implementList(IntList, int) declareList(SgidList, sgid_t)
                implementList(SgidList,
                              sgid_t) static double* insrc_buf_;  // Receives the interprocessor
...
static MapNode2PDbl* mk_svibuf() {
    rm_svibuf();
    if (!visources_ || visources_->count() == 0) {
        return NULL;
    }
        ...
        for (i = 0; i < n; ++i) {
            ...
            if (!pp) {
                fprintf(stderr,
                        ...
                        (long long) sgid);
                hoc_execerror(
                    "For multiple threads, the target pointer must reference a range variable of a "
                    "POINT_PROCESS.",
                    "Note that it is fastest to supply a reference to the POINT_PROCESS as the "
                    "first arg.");
            }
        }
...
void pargap_jacobi_setup(int mode) {
    ...
    if (mode == 0) {
        ...
        if (imped_current_type_count_ == 0 && targets_ && targets_->count() > 0) {
            ...
            if (ninst != targets_->count()) {
                ...
                hoc_execerror("number of gap junctions not equal to number of pc.transfer_var",
                              buf);
            }
        }
    }
    ...
    if (mode == 0) {  // setup
        if (visources_->count()) {
            vgap1 = new double[visources_->count()];
        }
        if (ttd && ttd->cnt) {
...
extern "C" {
void get_partrans_setup_info(int tid,
                             int& ntar,
                             int& nsrc,
                             int& type,
                             int& ix_vpre,

...
#define declareNrnHash(Table, Key, Value)                                        \
    struct NrnHashLT(Table){int operator()(Key i, Key j)                         \
                                const {return (key_to_hash(i) < key_to_hash(j)); \
    }                                                                            \
    }                                                                            \
    ;                                                                            \
                                                                                 \
    class NrnHashEntry(Table)                                                    \
        : public std::map<Key, Value, NrnHashLT(Table)>{};                       \
                                                                                 \
    class Table: public std::vector<NrnHashEntry(Table)> {                       \
      public:                                                                    \
        ...
        NrnHashEntry(Table) & at(unsigned long bucket) {                         \
            return *(begin() + bucket);                                          \
        }                                                                        \
        ...
        void remove(Key);                                                        \
        unsigned long hash(Key key) const {                                      \
            return (key_to_hash(key)) % size_;                                   \
        }                                                                        \
        long size_;                                                              \
    };
...
#define NrnHashIterate(Table, table, Value, value)                            \
    if (table)                                                                \
        for (long i__ = table->size_ - 1; i__ >= 0; --i__) {                  \

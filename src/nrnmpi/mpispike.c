...
static void make_spikebuf_type() {
	...
	guard(MPI_Get_address(&s, &addresses[0]));
	guard(MPI_Get_address(&(s.nspike), &addresses[1]));
	guard(MPI_Get_address(&(s.gid[0]), &addresses[2]));
	guard(MPI_Get_address(&(s.spiketime[0]), &addresses[3]));

	displacements[0] = addresses[1] - addresses[0];
	displacements[1] = addresses[2] - addresses[0];
	displacements[2] = addresses[3] - addresses[0];

	guard(MPI_Type_create_struct(3, block_lengths, displacements, typelist, &spikebuf_type));
	guard(MPI_Type_commit(&spikebuf_type));
}
#endif

#if 1
#define check(i) check_(i)
#else
#define check(i) /**/
#endif

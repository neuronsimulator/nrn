...
def generate_hoc_domain_from_py():
    ...
    py_domain = gen_from + pkgutil.get_data('sphinx.domains', 'python.py').decode('utf-8')
    ...
    hoc_domain = regex.sub(lambda mo: hoc_dict[mo.string[mo.start():mo.end()]], py_domain)

    # Write out HOC domain
    with open(os.path.join(os.path.abspath("."), 'domains/hocdomain.py'), 'w') as hoc_py:

...
connectivity 
    .. tab:: Python
        ...
        This is established via, e.g., ``child_sec.connect(parent_sec)`` or
        (with all options) ``child_sec.connect(parent_sec(parent_x), child_x)`` and
        defines the parent of the section, which end of the section is attached to
        the parent (``child_x`` must be 0 or 1; default is 0), and where on the parent the
        attachment takes place (default is 1). To avoid confusion, it is best to attach the
        0 end of a section to the 1 end of a parent. This way (assuming the soma is the root)
        smaller numbers point toward the soma.
...
The area of a segment is PI*diam*L/nseg (µm²) and the half-segment axial 
resistance is \ ``0.01*sec.Ra*(L/2/sec.nseg)/(PI*(seg.diam/2)^2)``. The 0.01 factor is necessary 
to convert ohm-cm micron/µm² to MegOhms. Ends of cylinders are not 
...
.. tab:: HOC
    ...
    .. code-block::
        ...
        L=1000 
        diam(0:1)=11:100 
        for (x) print x, diam(x), area(x), PI*diam(x)*L/nseg, ri(x), 0.01*Ra*(L/2/nseg)/(PI*(diam(x)/2)^2) 
...
This is because under a :func:`define_shape` with no pre-existing 
...
to a parent.  This will ensure that the ``sec(x).diam`` (Python and MATLAB syntax;
``sec.diam(x)`` in HOC) as x ranges from 0 to 1 
...
.. function:: pt3dclear
    ...
    .. tab:: Python
        ...
        Description:
            ...
            If ``section`` is not specified, the currently accessed section is assumed.
    ...
    .. tab:: HOC         
        ...
        Syntax:
            ``section buffersize = pt3dclear()``

            ``section buffersize = n.pt3dclear(buffersize)``
...
.. function:: pt3dadd
    ...
    .. tab:: HOC
        ...
        Syntax:
            ...
            ``section pt3dadd(xvec, yvec, zvec, dvec)``
        ...
        Description:
            ...
            list. Assume that successive additions increase the arc length 
            monotonically. When pt3d points exist in ``section`` they are used 
            to compute *diam* and *L*. When *diam* or *L* are changed and \ 
            ``pt3dconst()==0`` 
            the 3-d info is changed to be consistent with the new values of 
            *L* and *diam*. (Note: When *L* is changed, \ ``define_shape()`` should be executed 
            to adjust the 3-d info so that branches appear connected.) 
            The existence of a spine at this point is signaled 
            by a negative value for *d*. 
...
.. function:: pt3dinsert
    ...
    .. tab:: Python
        ...
        Syntax:
            ...
            ``section.pt3dinsert(i, x, y, z, diam)``
...
.. function:: pt3dchange
    ...
    .. tab:: Python
        ...
        Syntax:
            ...
            ``section.pt3dchange(i, x, y, z, diam)``

...
Text-based pages are written in reStructuredText (:file:`.rst`) and live under the :file:`docs/` directory. The structure of the documentation follows a clear hierarchy: installation, user guides, scripting references (Python and HOC), tutorials, RxD tutorials, and course materials. There is a long list of already existing material, thus ensure that when you are submitting a PR for a new file it does not already exist. When contributing a new page or editing an existing one, place the file in the appropriate subdirectory and add it to the relevant toctree in the nearest :file:`index.rst` file so that it appears in the navigation. When submitting a PR, ensure that only the relevant files are changed and that the content of adjacent documents is not inadvertently altered.
...
Use NEURON's own RST conventions. The existing documentation makes heavy use of `:class:`, `:func:`, `:ref:`, `:menuselection:`, `:file:`, and `:download:` roles. Using these consistently means that cross-references and the API docs stay linked and searchable. Look at existing :file:`.rst` files for patterns before writing new ones.
...
Screenshots are welcome but should be kept small, clearly cropped, and saved as PNG. Only use a screenshot when the visual genuinely adds information that a code block cannot convey — for example, a screenshot of the NEURON GUI's CellBuilder showing the layout of its panels is worth including because the visual structure is the point. By contrast, do not use a screenshot to show plain text output such as the result of `h.topology()` or a printed section list — a code block is preferable there because it is searchable, copyable, and takes up less space. If you want the reader to run code interactively, the content belongs in a :file:`.ipynb` notebook rather than an :file:`.rst` page; the :file:`.rst` page can then link to the notebook.

---

**Contributing Jupyter Notebooks and Tutorials**

The tutorials and interactive course materials live as Jupyter notebooks inside :file:`docs/tutorials/` and :file:`docs/rxd-tutorials/` (among other locations). These are valuable contributions for new users because they show working code alongside explanation, allowing people to understand a concept and execute it simultaneously.
...
For the reference page, videos are documented in :file:`.rst` files under :file:`docs/videos/`. Each entry typically includes the video title, a brief description of what is covered, the year or course it belongs to, and an embedded link or thumbnail. Follow the existing format in those files when adding a new entry.
...
Chapters and timestamps are extremely valuable. If you are uploading to YouTube, add chapter markers to the video description so viewers can jump to the specific part they need. When writing the :file:`.rst` entry in the NEURON docs, you can reproduce these timestamps as a brief table of contents in the text.
